...
use sym_poly_qsym::{key_stable, key_stable_monomial};
...
fn main() {
    ...
    println!("Stable key Kohnert limits in the monomial basis:");
    for alpha in &compositions {
        let expansion = key_stable_monomial::<i64>(alpha.parts());
        let terms = expansion
            .terms()
            .iter()
    ...
    println!();
    println!("Stable key Kohnert limits in the fundamental basis:");
    for alpha in compositions {
        let expansion = key_stable::<i64>(alpha.parts());
        let terms = expansion
            .terms()
            .iter()
