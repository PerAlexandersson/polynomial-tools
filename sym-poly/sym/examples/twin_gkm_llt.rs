...
fn main() {
    ...
    let areas = parse_input(&args).unwrap_or_else(|message| {
        eprintln!("{message}");
        eprintln!("usage:");
        eprintln!("  cargo run -p sym-poly-sym --example twin_gkm_llt");
        eprintln!("  cargo run -p sym-poly-sym --example twin_gkm_llt -- 4");
        eprintln!("  cargo run -p sym-poly-sym --example twin_gkm_llt -- 0,1,1");
        std::process::exit(2);
    });
