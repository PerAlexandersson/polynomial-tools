...
\begin{openproblem}\label{open:big_blocks_rooks}
Since the staircase standard rook polynomial is a reversed Touchard
polynomial, it is natural to ask for a rook-placement refinement whose
distribution records the statistic $\operatorname{bb}_j$.
At present we do not know a natural staircase-rook statistic that produces
the family $P_{n,j}(t)$ or $Q_{n,j}(t)$.
\end{openproblem}
