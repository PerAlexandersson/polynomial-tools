...
fn main() {
    let max_n: usize = std::env::args()
        ...
        .unwrap_or(7);
    println!(
        "INDUCTION STEP test: IH(λ) ⟹ IH(λ⁺) for all (λ,λ⁺) pairs, n ≤ {}\n",
        max_n
    );
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for j in 1..=m {
                for l in j..=m {
                    if !pz(&bd.a[j]) && !pz(&bd.w[l]) && !exact_interlaces(&bd.a[j], &bd.w[l]) {
                        ih_ok = false;
                    }
                    if j < l
                        && !pz(&bd.w[j])
                        && !pz(&bd.w[l])
                        && !exact_interlaces(&bd.w[j], &bd.w[l])
                    {
                        ih_ok = false;
                    }
                }
        ...
        println!(
            "n={}: pairs={}, A≼W tests={} (fail={}), W≼W tests={} (fail={}), IH_fail={}",
    ...
    println!(
        "W_j^+≼W_l^+ tests: {} (failures: {})  ← THE GAP",
        total_ww, ww_fails
    );
