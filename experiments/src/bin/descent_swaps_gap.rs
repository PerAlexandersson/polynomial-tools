...
fn main() {
    ...
    for n in 5..=max_n {
        ...
        for (&mask, class) in &by_descent {
            ...
            let g_polys: Vec<(u8, Vec<i64>)> = g_by_r
                .iter()
            ...
            for (_, poly) in &g_polys {
                all_polys.push(poly);
            }
            for (_, _, poly) in &fq_polys {
                all_polys.push(poly);
            }
        ...
        println!(
            "n={}: G_r vs L^(p) {}/{}, G_r vs F~_q {}/{}, full family {}/{}",
            n, n_gr_lp_ok, n_gr_lp_total, n_gr_fq_ok, n_gr_fq_total, n_family_ok, n_family_total
        );
    }
    ...
    println!(
        "Full {{G_r, F~_q}} family: {}/{}",
        all_gr_fq_family_ok, all_gr_fq_family_total
    );
    println!();
