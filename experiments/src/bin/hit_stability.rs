...
fn perm_complex(mat: &[Vec<C64>]) -> C64 {
    ...
    loop {
        ...
        for i in 0..n {
            prod = prod.mul(mat[i][indices[i]]);
        }
        result = result.add(prod);
        if !next_perm(&mut indices) {
            break;
        }
    }
    result
...
fn main() {
    ...
    for &s in &test_points {
        for &t in &test_points {
            ...
            if p.norm2() < 1e-20 {
                println!(
                    "  ZERO at s={:.2}+{:.2}i, t={:.2}+{:.2}i: F={:.6}+{:.6}i",
    ...
    if !found_zero {
        println!("  No zeros found (but we know s=t=i gives zero)");
    }
    // Exact check at s=t=i
    let s = C64::new(0.0, 1.0);
    let t = C64::new(0.0, 1.0);
    let mat = vec![vec![C64::new(1.0, 0.0), s], vec![t, C64::new(1.0, 0.0)]];
    let p = perm_complex(&mat);
    println!(
        "  At s=t=i: F = {:.6} + {:.6}i (norm²={:.2e})",
        p.re,
        p.im,
        p.norm2()
    );
    ...
    if min_norm_all > 1e-10 {
        println!(
            "  CONCLUSION: No zeros found in upper half-plane → consistent with stability <<<"
        );
    } else {
        ...
        for _ in 0..100 {
            ...
            for &s_val in &test_points {
                for &t_val in &test_points {
                    ...
                    let p = perm_complex(&mat);
                    rand_tests += 1;
                    if p.norm2() < rand_min_norm {
                        rand_min_norm = p.norm2();
                    }
                }
            }
        }
