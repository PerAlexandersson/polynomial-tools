...
fn main() {
    ...
    println!(
        ...
        fk_rr[0] - fk_rr[1],
        fk_rr[0],
        if fk_rr[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "F_j≼F_l (j<l, fwd): {}/{} {}",
        fk_fwd[0] - fk_fwd[1],
        fk_fwd[0],
        if fk_fwd[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "F_l≼F_j (j<l, rev): {}/{} {}",
        fk_rev[0] - fk_rev[1],
        fk_rev[0],
        if fk_rev[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "G_j≼G_l (j<l, fwd): {}/{} {}",
        gk_fwd[0] - gk_fwd[1],
        gk_fwd[0],
        if gk_fwd[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "G_l≼G_j (j<l, rev): {}/{} {}",
        gk_rev[0] - gk_rev[1],
        gk_rev[0],
        if gk_rev[1] == 0 { "✓" } else { "FAIL" }
    );
