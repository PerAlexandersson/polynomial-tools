...
fn shifted_exc_bruteforce(perm_prefix: &[u8], remaining: &[u8], s: i32, n: usize) -> Vec<i64> {
    if remaining.is_empty() {
        let exc = (0..n).filter(|&i| perm_prefix[i] as i32 > (i as i32 + 1) + s).count();
    ...
    for (idx, &val) in remaining.iter().enumerate() {
        ...
        next_rem.remove(idx);
        result = pa(&result, &shifted_exc_bruteforce(&next_perm, &next_rem, s, n));
    }
    result
}
...
fn shifted_exc_refined(n: usize, s: i32) -> Vec<Vec<Vec<i64>>> {
    ...
    fn gen(prefix: &[u8], remaining: &[u8], n: usize, s: i32, result: &mut Vec<Vec<Vec<i64>>>) {
        if remaining.is_empty() {
            ...
            let exc = (0..n).filter(|&i| prefix[i] as i32 > (i as i32 + 1) + s).count();
            while result[j][l].len() <= exc { result[j][l].push(0); }
            result[j][l][exc] += 1;
            return;
        }
        for (idx, &val) in remaining.iter().enumerate() {
...
fn main() {
    ...
    for n in 3..=5 {
        ...
        for s in 0..n as i32 {
            ...
            for j in 1..=n { for l in 1..=n { if j!=l { by_first[j] = pa(&by_first[j], &refined[j][l]); by_last[l] = pa(&by_last[l], &refined[j][l]); } } }
            print!("    Σ_l: ");
            for j in 1..=n { if !pz(&by_first[j]) { print!("F({},*) = {}  ", j, format_poly(&pt(&by_first[j]))); } }
            println!();
            print!("    Σ_j: ");
            for l in 1..=n { if !pz(&by_last[l]) { print!("F(*,{}) = {}  ", l, format_poly(&pt(&by_last[l]))); } }
            println!();
        }
        println!();
    }
