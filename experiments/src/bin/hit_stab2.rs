...
fn sub_partitions(lam: &[u8]) -> Vec<Vec<u8>> {
    let n = lam.len();
    let mut res = Vec::new();
    let mut mu = vec![0u8; n];
    fn gen(l: &[u8], m: &mut Vec<u8>, p: usize, mx: u8, r: &mut Vec<Vec<u8>>) {
        if p == l.len() {
            r.push(m.clone());
            return;
        }
    ...
    gen(lam, &mut mu, 0, lam[0], &mut res);
    res
}
fn main() {
    ...
    println!("  Worst: {}", worst);
    if min_norm > 1e-6 {
        println!("  STABLE: no near-zeros in upper half-plane <<<");
    } else {
        println!("  UNSTABLE: near-zero detected!");
    }
}
