...
fn end_type(pi: &[u8]) -> char {
    let n = pi.len();
    if n <= 1 { return 'a'; } // convention: length 1 is ascending
    if pi[n - 2] < pi[n - 1] { 'a' } else { 'd' }
}
...
fn main() {
    ...
    for n in 1..=max_n {
        ...
        for board in &boards {
            ...
            for pi in &ideal {
                ...
                // Unsplit
                let poly_all = if st == 'd' { &mut d_all[k] } else { &mut u_all[k] };
                while poly_all.len() <= pk { poly_all.push(0); }
                poly_all[pk] += 1;
                ...
                while poly_ref.len() <= pk { poly_ref.push(0); }
                poly_ref[pk] += 1;

                // Last-entry tracking
                if last <= m {
                    while pkl[k][last].len() <= pk { pkl[k][last].push(0); }
                    pkl[k][last][pk] += 1;
                }
            }
            ...
            for j in 1..=m {
                for l in 1..=m {
                    if !pz(&a_all[j]) && !pz(&w_all[l]) {
                        aw_all[0] += 1;
                        if !interlaces(&a_all[j], &w_all[l]) { aw_all[1] += 1; }
                    }
                    if j <= l && j < l {
                        if !pz(&w_all[j]) && !pz(&w_all[l]) {
                            ww_all[0] += 1;
                            if !interlaces(&w_all[j], &w_all[l]) { ww_all[1] += 1; }
                        }
                    }
                }
            }
                ...
                for l in 1..=m {
                    ...
                    if !pz(&a_ea[j]) && !pz(&w_ea[l]) {
                        ...
                        if !interlaces(&a_ea[j], &w_ea[l]) {
                            aw_ea[1] += 1;
                            if aw_ea[1] <= 3 {
                                println!("FAIL A_j^ea << W_l^ea: board={:?} j={} l={}", board, j, l);
                    ...
                    if !pz(&a_ed[j]) && !pz(&w_ed[l]) {
                        ...
                        if !interlaces(&a_ed[j], &w_ed[l]) {
                            aw_ed[1] += 1;
                            if aw_ed[1] <= 3 {
                                println!("FAIL A_j^ed << W_l^ed: board={:?} j={} l={}", board, j, l);
                    ...
                    if !pz(&a_ea[j]) && !pz(&w_ed[l]) {
                        ...
                        if !interlaces(&a_ea[j], &w_ed[l]) {
                            aw_ea_ed[1] += 1;
                            if aw_ea_ed[1] <= 3 {
                                println!("FAIL A_j^ea << W_l^ed: board={:?} j={} l={}", board, j, l);
                                println!("  A_j^ea = {}", pf(&a_ea[j]));
                                println!("  W_l^ed = {}", pf(&w_ed[l]));
                            }
                        }
                    ...
                    if !pz(&a_ed[j]) && !pz(&w_ea[l]) {
                        ...
                        if !interlaces(&a_ed[j], &w_ea[l]) {
                            aw_ed_ea[1] += 1;
                            if aw_ed_ea[1] <= 3 {
                                println!("FAIL A_j^ed << W_l^ea: board={:?} j={} l={}", board, j, l);
                                println!("  A_j^ed = {}", pf(&a_ed[j]));
                                println!("  W_l^ea = {}", pf(&w_ea[l]));
                            }
                        }
                    ...
                    if j < l {
                        if !pz(&w_ea[j]) && !pz(&w_ea[l]) {
                            ...
                            if !interlaces(&w_ea[j], &w_ea[l]) {
                                ww_ea[1] += 1;
                                if ww_ea[1] <= 3 {
                                    println!("FAIL W_j^ea << W_l^ea: board={:?} j={} l={}", board, j, l);
                        ...
                        if !pz(&w_ed[j]) && !pz(&w_ed[l]) {
                            ...
                            if !interlaces(&w_ed[j], &w_ed[l]) {
                                ww_ed[1] += 1;
                                if ww_ed[1] <= 3 {
                                    println!("FAIL W_j^ed << W_l^ed: board={:?} j={} l={}", board, j, l);
                        ...
                        if !pz(&w_ea[j]) && !pz(&w_ed[l]) {
                            ...
                            if !interlaces(&w_ea[j], &w_ed[l]) {
                                ww_ea_ed[1] += 1;
                                if ww_ea_ed[1] <= 3 {
                                    println!("FAIL W_j^ea << W_l^ed: board={:?} j={} l={}", board, j, l);
                                    println!("  W_j^ea = {}", pf(&w_ea[j]));
                                    println!("  W_l^ed = {}", pf(&w_ed[l]));
                                }
                            }
                    ...
                    if !pz(&d_ea[j]) && !pz(&u_ea[l]) {
                        ...
                        if !interlaces(&d_ea[j], &u_ea[l]) {
                            du_ea[1] += 1;
                            if du_ea[1] <= 3 {
                                println!("FAIL D_j^ea << U_l^ea: board={:?} j={} l={}", board, j, l);
                            }
                        }
                    }
                    if !pz(&d_ed[j]) && !pz(&u_ed[l]) {
                        ...
                        if !interlaces(&d_ed[j], &u_ed[l]) {
                            du_ed[1] += 1;
                            if du_ed[1] <= 3 {
                                println!("FAIL D_j^ed << U_l^ed: board={:?} j={} l={}", board, j, l);
                            }
                        }
                    }
                    ...
                    if j <= l {
                        if !pz(&u_ea[j]) && !pz(&u_ea[l]) {
                            ...
                            if !interlaces(&u_ea[j], &u_ea[l]) {
                                uu_ea[1] += 1;
                                if uu_ea[1] <= 3 {
                                    println!("FAIL U_j^ea << U_l^ea: board={:?} j={} l={}", board, j, l);
                                }
                            }
                        }
                        if !pz(&u_ed[j]) && !pz(&u_ed[l]) {
                            ...
                            if !interlaces(&u_ed[j], &u_ed[l]) {
                                uu_ed[1] += 1;
                                if uu_ed[1] <= 3 {
                                    println!("FAIL U_j^ed << U_l^ed: board={:?} j={} l={}", board, j, l);
                                }
                            }
                        }
                    }
            ...
            for k in 1..=m {
                if !pz(&a_ea[k]) && !pz(&a_ed[k]) {
                    ...
                    ak_ea_ed[0] += 1;
                    if !ea_ed { ak_ea_ed[1] += 1; }
                    ak_ed_ea[0] += 1;
                    if !ed_ea { ak_ed_ea[1] += 1; }
                    if !ea_ed && !ed_ea {
                        ak_either[1] += 1;
                        if ak_either[1] <= 3 {
                            println!("FAIL neither A_k^ea << A_k^ed nor reverse: board={:?} k={}", board, k);
                ...
                for l in 1..=m {
                    if !pz(&pkl[k][l]) {
                        ...
                        if !is_real_rooted(&pkl[k][l]) {
                            pkl_rr[1] += 1;
                            if pkl_rr[1] <= 3 {
                                println!("FAIL P_{{{},{}}} not real-rooted: board={:?} poly={}", k, l, board, pf(&pkl[k][l]));
                            }
                        }
                    }
                }
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for pi in &ideal {
                ...
                while poly_us.len() <= pk { poly_us.push(0); }
                poly_us[pk] += 1;
            ...
            for k in 1..=m {
                ...
                if pt(&d_sum) != pt(&d_k[k]) || pt(&u_sum) != pt(&u_k[k]) {
                    ...
                    if decouple_fail < 3 {
                        ...
                        println!("  D_k = {} vs D_k^ea + D_k^ed = {}", pf(&d_k[k]), pf(&d_sum));
                        println!("  U_k = {} vs U_k^ea + U_k^ed = {}", pf(&u_k[k]), pf(&u_sum));
                    }
                }
            }
            if ok {
    ...
    println!("Decoupling consistency (D/U split sums match): {}/{} boards",
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for pi in &ideal {
                ...
                let first = pi[0];
                let rest: Vec<u8> = pi[1..].iter().map(|&x| {
                    if x > first { x - 1 } else { x }
                }).collect();
                ...
                if rest.len() >= 2 {
                    ...
                    if et_full == et_rest {
                        ...
                        preserve_fail += 1;
                        if preserve_fail <= 5 {
                            println!("  MISMATCH: pi={:?} end_type={} rest={:?} end_type={}",
    ...
    println!("  Pass: {} ({:.1}%)", preserve_pass,
             if preserve_checks > 0 { 100.0 * preserve_pass as f64 / preserve_checks as f64 } else { 0.0 });
    if preserve_fail > 0 {
        println!("  FAIL: {} - end-type NOT preserved under first-entry removal!", preserve_fail);
    } else {
    ...
    for n in 1..=std::cmp::min(max_n, 5) {
        ...
        for board in &boards {
            ...
            if !is_312_avoiding(&perm) { continue; }
            let m = (board[0] as usize).min(n);
            if m < 2 || n < 3 { continue; } // skip trivial cases
            ...
            let has_data = (1..=m).any(|k|
                !pz(&d_ea[k]) || !pz(&d_ed[k]) || !pz(&u_ea[k]) || !pz(&u_ed[k])
            );
            ...
            for k in 1..=m {
                ...
                if pz(&a_ea_k) && pz(&a_ed_k) { continue; }
                println!("  k={}:", k);
                if !pz(&u_ea[k]) { println!("    U_k^ea = {}", pf(&u_ea[k])); }
                if !pz(&u_ed[k]) { println!("    U_k^ed = {}", pf(&u_ed[k])); }
                if !pz(&d_ea[k]) { println!("    D_k^ea = {}", pf(&d_ea[k])); }
                if !pz(&d_ed[k]) { println!("    D_k^ed = {}", pf(&d_ed[k])); }
                if !pz(&a_ea_k) { println!("    A_k^ea = {}", pf(&a_ea_k)); }
                if !pz(&a_ed_k) { println!("    A_k^ed = {}", pf(&a_ed_k)); }
                if !pz(&w_ea_k) { println!("    W_k^ea = {}", pf(&w_ea_k)); }
                if !pz(&w_ed_k) { println!("    W_k^ed = {}", pf(&w_ed_k)); }
            }
            println!();
        }
    }
    ...
    println!("=== DECOUPLING ===\n");
    println!("  Split consistency (D/U sums): {}/{}", decouple_match, decouple_boards_checked);
    println!("  End-type preservation under first-entry removal: {}/{}",
