        ...
        for board in &gen_boards(n) {
            ...
            for i in 0..subs.len() {
                for j in 0..subs.len() {
                    ...
                    let f = &polys[i]; let g = &polys[j];
                    if pz(f) || pz(g) { continue; }
                    // H_μ ≪ H_{μ'} (smaller sub-partition on the left)
                    mono[0] += 1;
                    if !interlaces(f, g) { mono[1] += 1; }
                    // H_{μ'} ≪ H_μ (reverse)
                    mono_rev[0] += 1;
                    if !interlaces(g, f) { mono_rev[1] += 1; }
                    ...
                    if diff.len() == 1 && subs[j][diff[0]] == subs[i][diff[0]] + 1 {
                        mono_cover[0] += 1;
                        if !interlaces(f, g) { mono_cover[1] += 1; }
                        mono_cover_rev[0] += 1;
                        if !interlaces(g, f) { mono_cover_rev[1] += 1; }
                        // Wagner: H_{μ'} ≪ t·H_μ?
                        let tf = pmt(f);
                        wagner_cover[0] += 1;
                        if !interlaces(g, &tf) { wagner_cover[1] += 1; }
                    }
                }
            }
        }
