...
fn main() {
    ...
    for &(stat_name, stat) in &stats {
        ...
        for (pi, pat) in patterns5.iter().enumerate() {
            ...
            let first_fail = polys.iter().enumerate()
                .find(|(_, p)| !polynomial::is_real_rooted(p))
                .map(|(i, _)| i + 1);

            let key: String = polys.iter()
                .map(|p| p.iter().map(|c| c.to_string()).collect::<Vec<_>>().join(","))
                .collect::<Vec<_>>().join("|");

            equiv_classes.entry(key.clone()).or_default().push(label.clone());
            class_data.entry(key).or_insert((polys, first_fail));

            if (pi + 1) % 20 == 0 {
                eprint!("\r  {}/{} patterns done...          ", pi + 1, patterns5.len());
                io::stderr().flush().ok();
            }
        }
        ...
        for (key, pats) in &equiv_classes {
            ...
            println!("  Class {:2}: [size {:3}] Av_{{{}}}    {}", class_id, size, pats_display, rr);
        }
        ...
        for (key, pats) in &equiv_classes {
            ...
            eprint!("\r  Searching rec for Av_{{{}}} + {} ...          ", pats_display, stat_name);
            io::stderr().flush().ok();
            ...
            if let Some(result) = find_recurrence_adaptive(polys, &opts) {
                println!("  Av_{{{}}} + {}: {}", pats_display, stat_name, result.recurrence);
                searched += 1;
            }
        }
        if searched == 0 {
            ...
            let (polys, _) = class_data.get(
                &equiv_classes.iter().find(|(_, v)| std::ptr::eq(v.as_slice(), largest.as_slice())).unwrap().0.clone()
            ).unwrap();
            let opts = AdaptiveSearchOptions {
                max_rec_len: 2, max_var_deg: 2, max_idx_deg: 2, max_diff_deg: 1,
                verbose: false, ..Default::default()
            ...
            eprint!("\r  Searching rec for largest class Av_{{{}}} + {} ...          ", pats_display, stat_name);
            io::stderr().flush().ok();
            match find_recurrence_adaptive(polys, &opts) {
                Some(r) => println!("  Av_{{{}}} + {}: {}", pats_display, stat_name, r.recurrence),
