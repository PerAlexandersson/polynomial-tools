...
fn check_rr(p: &Poly) -> Option<bool> {
    if deg(p) <= 1 { return Some(true); }
    if let Some(p64) = poly_to_i64(p) { return Some(is_real_rooted(&p64)); }
    None
}
...
fn compute_with_weights(mu: &[usize], d: usize, max_n: usize,
                        weight_fn: impl Fn(usize) -> Poly) -> Vec<Poly> {
    let ell = mu.len() - 1;
...
fn check_sequence(name: &str, polys: &[Poly], max_print: usize) {
    ...
    for (n, p) in polys.iter().enumerate() {
        ...
        match check_rr(p) {
            Some(true) => last_ok = n,
            Some(false) => { if first_fail.is_none() { first_fail = Some(n); } break; }
            None => break,
        }
    }
    match first_fail {
        None => println!("  => RR for all n <= {} (deg {})\n", last_ok, deg(&polys[last_ok])),
...
fn main() {
    ...
    let p_polys = compute_with_weights(&mu, d, max_n, |v| {
        if v > 0 { vec![0, 1] } else { vec![1] }  // t for tiles, 1 for gaps
    });
    ...
    if d > 1 {
        println!("--- T_n(s): bottom-row refined (s marks row-{} tiles) ---", d);
        let t_polys = compute_with_weights(&mu, d, max_n, |v| {
            if v == 0 { vec![1] }
            else if v == d { vec![0, 1] }  // bottom row: weight s
            else { vec![1] }
        });
    ...
    for n in 1..=max_n {
        ...
        while qn.len() > 1 && *qn.last().unwrap() == 0 { qn.pop(); }
        q_polys[n] = qn;
    }
    ...
    if d >= 3 {
        let mid = (d + 1) / 2;
        println!("--- M_n(s): mid-row refined (s marks row-{} tiles) ---", mid);
        let m_polys = compute_with_weights(&mu, d, max_n, |v| {
            if v == 0 { vec![1] }
            else if v == mid { vec![0, 1] }
            else { vec![1] }
        });
