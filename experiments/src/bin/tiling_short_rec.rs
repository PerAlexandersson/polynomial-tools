...
fn search_short_recurrence(polys: &[Poly], max_order: usize) {
    ...
    for n in 1..n_polys {
        ...
        for (i, &c) in polys[n].iter().enumerate() {
            d[i] += c;
        }
        for (i, &c) in polys[n - 1].iter().enumerate() {
            d[i] -= c;
    ...
    for order in 1..=8 {
        ...
        for n in start..diffs.len() {
            ...
            while residual.len() > 1 && *residual.last().unwrap() == 0 {
                residual.pop();
            }

            if poly_is_zero(&residual) {
                continue;
            }
    ...
    for n in 1..diffs.len() {
        ...
        while rhs.len() > 1 && *rhs.last().unwrap() == 0 {
            rhs.pop();
        }
        if !poly_is_zero(&rhs) {
            r_betas[n] = rhs.clone();
...
fn check_interlacing_depth(polys: &[Poly], max_k: usize) {
    ...
    for k in 1..=max_k {
        ...
        for n in 0..=(n_max - k) {
            let f = match poly_to_i64(&polys[n]) {
                Some(v) => v,
                None => break,
            };
            let g = match poly_to_i64(&polys[n + k]) {
                Some(v) => v,
                None => break,
            };
        ...
        match first_fail {
            None => println!(
                "  k={}: P_n << P_{{n+{}}} holds for all n <= {} (where degrees are compatible)",
                k, k, last_checked
            ),
            Some(n) => println!(
                "  k={}: FAILS at n={} (deg P_{}={}, deg P_{}={})",
                k,
                n,
                n,
                deg(&polys[n]),
