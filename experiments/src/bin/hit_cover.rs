        ...
        for board in &gen_boards(n) {
            ...
            for i in 0..subs.len() {
                for j in 0..subs.len() {
                    ...
                    let diff: Vec<(usize, u8)> = (0..n)
                        .filter(|&k| subs[i][k] != subs[j][k])
                        .map(|k| (k, subs[j][k] - subs[i][k]))
                        .collect();
                    ...
                    if diff.len() == 1 && diff[0].1 == 1 {
                        ...
                        if !pz(&d) {
                            ...
                            if d_at_1 == 0 {
                                ...
                                let c = pt(&c);
                                cofactor_rr[0] += 1;
                                if !pz(&c) && (is_real_rooted(&c) || c.len() <= 2) {
                                    cofactor_rr[1] += 1;
                                }
                                // Shift lemma: C ≪ H_{μ'}?
                                if !pz(&c) && !pz(&polys[j]) {
                                    shift_lemma[0] += 1;
                                    if interlaces(&c, &polys[j]) {
                                        shift_lemma[1] += 1;
                                    }
                                }
                    ...
                    if diff.len() == 2 && diff.iter().all(|&(_, d)| d == 1) {
                        if !pz(&polys[i]) && !pz(&polys[j]) {
                            multi2[0] += 1;
                            if interlaces(&polys[i], &polys[j]) {
                                multi2[1] += 1;
                            }
                            multi2_rev[0] += 1;
                            if interlaces(&polys[j], &polys[i]) {
                                multi2_rev[1] += 1;
                            }
                        }
                    }
