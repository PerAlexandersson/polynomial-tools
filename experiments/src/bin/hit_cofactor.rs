...
fn interlaces(f: &[i64], g: &[i64]) -> bool { let f = pt(f); let g = pt(g); if pz(&f) { return true; } if pz(&g) { return false; } check_weak_interlacing(&f, &g).unwrap_or(false) }
...
fn hit_poly_on_ideal(ideal: &[Vec<u8>], mu: &[u8]) -> Vec<i64> {
    let n = mu.len(); let mut p = vec![0i64];
...
fn cofactor_poly(ideal: &[Vec<u8>], mu: &[u8], row_j: usize, col: usize) -> Vec<i64> {
    ...
    for sigma in ideal {
        ...
        for i in 0..n {
            ...
            if sigma[i] as usize > mu[i] as usize { hits += 1; }
        }
...
fn main() {
    ...
    for n in 2..=max_n {
        for board in &gen_boards(n) {
            ...
            for mu in &subs {
                for j in 0..n { // row j (0-indexed)
                    if mu[j] >= board[j] { continue; } // can't increment
                    ...
                    if pz(&c_j) { continue; }
                    // Degree gap
                    let dh = pdeg(&h_mp); let dc = pdeg(&c_j);
                    deg_gap[0] += 1;
                    if dh <= dc + 1 { deg_gap[1] += 1; }
                    // C_j ≪ H_{μ'}
                    cj_lt_hmp[0] += 1;
                    if interlaces(&c_j, &h_mp) { cj_lt_hmp[1] += 1; }
                    // Cofactor comparisons
                    for k in 1..=m {
                        if k == mu[j] as usize + 1 { continue; } // skip C_j itself
                        ...
                        if k <= mu[j] as usize {
                            ...
                            cj_lt_cjk[0] += 1;
                            if interlaces(&c_j, &c_jk) { cj_lt_cjk[1] += 1; }
                        } else {
                            // k > μ_j+1: should have C_{j,k} ≪ C_j and C_j ≪ t·C_{j,k}
                            cjk_lt_cj[0] += 1;
                            if interlaces(&c_jk, &c_j) { cjk_lt_cj[1] += 1; }
                            let t_cjk = pmt(&c_jk);
                            cj_lt_tcjk[0] += 1;
                            if interlaces(&c_j, &t_cjk) { cj_lt_tcjk[1] += 1; }
                        }
                    }
                }
            }
    ...
    let show = |name: &str, c: [u64;2]| { if c[0]==0 { println!("  {}: (no data)", name); } else if c[1]==c[0] { println!("  {}: {}/{} ALL PASS <<<", name, c[0], c[0]); } else { println!("  {}: {}/{} pass ({} FAIL)", name, c[1], c[0], c[0]-c[1]); } };
