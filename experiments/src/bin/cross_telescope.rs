        ...
        for board in &gen_boards(n) {
            ...
            for l in 2..=m {
                if !pz(&sum_w[l]) && !pz(&sum_w[l - 1]) {
                    sw_sw[0] += 1;
                    if !interlaces(&sum_w[l], &sum_w[l - 1]) {
                        sw_sw[1] += 1;
                    }
                    sw_sw_rev[0] += 1;
                    if !interlaces(&sum_w[l - 1], &sum_w[l]) {
                        sw_sw_rev[1] += 1;
                    }
                }
                // Σ W^l ≪ W_i^{l-1}
                for i in 1..=m {
                    if !pz(&sum_w[l]) && !pz(&w_jl[i][l - 1]) {
                        sw_wi[0] += 1;
                        if !interlaces(&sum_w[l], &w_jl[i][l - 1]) {
                            sw_wi[1] += 1;
                        }
                    }
                }
                ...
                if !pz(&u_jl[1][l]) && !pz(&sum_w[l - 1]) {
                    u1_sw[0] += 1;
                    if !interlaces(&u_jl[1][l], &sum_w[l - 1]) {
                        u1_sw[1] += 1;
                    }
                }
            }
