...
fn main() {
    ...
    for n in 10..=17u8 {
        ...
        let mut worst_inv = 0;
        for w in &perms {
            let inv: usize = (0..w.len())
                .flat_map(|i| (i + 1..w.len()).filter(move |&j| w[i] > w[j]))
                .count();
            if inv > worst_inv {
                worst_inv = inv;
                worst = Some(w.clone());
        ...
        let mut inv_perms: Vec<(usize, &Vec<u8>)> = perms
            .iter()
            .map(|w| {
                let inv: usize = (0..w.len())
                    .flat_map(|i| (i + 1..w.len()).filter(move |&j| w[i] > w[j]))
        ...
        if fail_count == 0 {
            println!(
                "n={}: top {}  perms (by inv) all pass RR (max inv={})",
                n, top, worst_inv
            );
        } else {
