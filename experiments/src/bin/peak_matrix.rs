...
fn print_recursion_matrix(board_name: &str, m: usize) {
    ...
    println!("\n--- Recursion matrix for {} (m={}, m'={}) ---", board_name, m, m_prime);
    println!("Input  (cols): A_{}, ..., A_1, W_{}, ..., W_1", m, m);
    println!("Output (rows): A_{}, ..., A_1, W_1, ..., W_{}", m_prime, m_prime);
    println!();

    // Column headers
    let mut col_labels = vec![];
    for j in (1..=m).rev() { col_labels.push(format!("A{}", j)); }
    for j in (1..=m).rev() { col_labels.push(format!("W{}", j)); }
    let ncols = 2 * m;

    // Row labels
    let mut row_labels = vec![];
    for k in (1..=m_prime).rev() { row_labels.push(format!("A{}+", k)); }
    for k in 1..=m_prime { row_labels.push(format!("W{}+", k)); }
    let nrows = 2 * m_prime;
    ...
    // Print header
    print!("{:>6}", "");
    for l in &col_labels { print!("{:>4}", l); }
    println!();

    for (i, row) in matrix.iter().enumerate() {
        print!("{:>6}", row_labels[i]);
        for entry in row { print!("{:>4}", entry); }
        println!();
    }
}
...
fn main() {
    ...
    println!("=== Interlacing sequence conjecture + recursion matrix, n <= {} ===", max_n);
    ...
    for n in 1..=max_n {
        ...
        for board in &boards {
            ...
            for k in 1..=m { for j in 1..k {
                if !pz(&a[k]) && !pz(&a[j]) {
                    aa_rev[0] += 1;
                    if !exact_interlaces(&a[k], &a[j]) {
                        aa_rev[1] += 1;
                        if aa_rev[1] <= 5 {
                            println!("FAIL AA_rev({},{}) board={:?}: {} << {}", k, j, board, pf(&a[k]), pf(&a[j]));
                        }
                    }
                }
            } }
            ...
            for j in 1..=m { for l in 1..=m {
                if !pz(&a[j]) && !pz(&w[l]) {
                    ...
                    if !exact_interlaces(&a[j], &w[l]) {
                        aw_all[1] += 1;
                        if aw_all[1] <= 5 {
                            println!("FAIL AW({},{}) board={:?}: {} << {}", j, l, board, pf(&a[j]), pf(&w[l]));
                        }
                    }
                }
            } }
            ...
            for j in 1..=m { for l in j..=m {
                if j < l && !pz(&w[j]) && !pz(&w[l]) {
                    ...
                    if !exact_interlaces(&w[j], &w[l]) {
                        ww_ord[1] += 1;
                        if ww_ord[1] <= 5 {
                            println!("FAIL WW({},{}) board={:?}: {} << {}", j, l, board, pf(&w[j]), pf(&w[l]));
                        }
                    }
                }
            } }
            ...
            // f_i << f_j for all i < j in this ordering
            {
                ...
                let mut seq_ok = true;
                for i in 0..seq.len() { for j in i+1..seq.len() {
                    ...
                    if full_seq[1] <= 5 {
                        ...
                        for i in 0..seq.len() { for j in i+1..seq.len() {
                            if !pz(seq[i]) && !pz(seq[j]) && !exact_interlaces(seq[i], seq[j]) {
                                println!("  {} << {} fails: {} << {}", seq_labels[i], seq_labels[j], pf(seq[i]), pf(seq[j]));
                            }
                        } }
                    }
                }
    ...
    let show = |name: &str, c: [u64; 2]| {
        ...
        else { println!("  {}: {}/{} pass {}", name, c[0] - c[1], c[0], if c[1] == 0 { "ALL PASS" } else { "FAILURES" }); }
    };
    ...
    for board in &example_boards {
        let perm = board_to_perm(board);
        if !is_312_avoiding(&perm) { println!("Board {:?}: not 312-avoiding, skipping", board); continue; }
        let (a, w) = compute_aw(board);
        let m = a.len() - 1;
        ...
        print!("  Sequence: ");
        for k in (1..=m).rev() { print!("A{}, ", k); }
        for k in 1..=m {
            if k < m { print!("W{}, ", k); } else { print!("W{}", k); }
        }
        println!();
        // Check pairwise
        let mut seq: Vec<&Vec<i64>> = Vec::new();
        let mut labels: Vec<String> = Vec::new();
        for k in (1..=m).rev() { seq.push(&a[k]); labels.push(format!("A{}", k)); }
        for k in 1..=m { seq.push(&w[k]); labels.push(format!("W{}", k)); }
        let mut all_ok = true;
        for i in 0..seq.len() { for j in i+1..seq.len() {
            if !pz(seq[i]) && !pz(seq[j]) {
                let ok = exact_interlaces(seq[i], seq[j]);
                if !ok {
                    println!("    {} << {}: FAIL ({} << {})", labels[i], labels[j], pf(seq[i]), pf(seq[j]));
                    all_ok = false;
                }
            }
        } }
        if all_ok { println!("  All pairwise interlacings hold."); }
        println!();
    }
}
