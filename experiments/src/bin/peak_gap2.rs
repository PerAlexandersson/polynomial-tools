...
fn main() {
    let max_n: usize = std::env::args()
        ...
        .unwrap_or(6);
    println!(
        "=== DU proof gap (corrected float-based checker), n ≤ {} ===\n",
        max_n
    );
    ...
    println!(
        "  interlaces(f,g) = {} (paper f≪g: should be TRUE)",
        interlaces(&f, &g)
    );
    println!(
        "  interlaces(g,f) = {} (paper g≪f: should be FALSE)",
        interlaces(&g, &f)
    );
    ...
    println!(
        "  interlaces(f,g) = {} (paper f≪g: should be FALSE)",
        interlaces(&f2, &g2)
    );
    println!(
        "  interlaces(g,f) = {} (paper g≪f: should be TRUE)",
        interlaces(&g2, &f2)
    );
    println!();
        ...
        for board in &boards {
            ...
            for j in 1..=m {
                for l in j..=m {
                    if !pz(&bd.a[j]) && !pz(&bd.w[l]) {
                        aw_ord[0] += 1;
                        if !interlaces(&bd.a[j], &bd.w[l]) {
                            aw_ord[1] += 1;
                            if aw_ord[1] <= 3 {
                                println!(
                                    "FAIL AW: A_{}≪W_{} board {:?}: {} vs {}",
                                    j,
                                    l,
                                    board,
                                    pf(&bd.a[j]),
                    ...
                    if j < l && !pz(&bd.w[j]) && !pz(&bd.w[l]) {
                        ww_ord[0] += 1;
                        if !interlaces(&bd.w[j], &bd.w[l]) {
                            ww_ord[1] += 1;
                            if ww_ord[1] <= 3 {
                                println!("FAIL WW: W_{}≪W_{} board {:?}", j, l, board);
                            }
                        }
                    }
                    if !pz(&bd.u[j]) && !pz(&bd.u[l]) {
                        uu_ord[0] += 1;
                        if !interlaces(&bd.u[j], &bd.u[l]) {
                            uu_ord[1] += 1;
                            if uu_ord[1] <= 3 {
                                println!(
                                    "FAIL UU: U_{}≪U_{} board {:?}: {} vs {}",
                                    j,
                                    l,
                                    board,
                                    pf(&bd.u[j]),
            ...
            for j in 1..=m {
                for l in 1..=m {
                    if !pz(&bd.d[j]) && !pz(&bd.u[l]) {
                        du_all[0] += 1;
                        if !interlaces(&bd.d[j], &bd.u[l]) {
                            du_all[1] += 1;
                            if du_all[1] <= 3 {
                                println!(
                                    "FAIL DU: D_{}≪U_{} board {:?}: {} vs {}",
                                    j,
                                    l,
                                    board,
                                    pf(&bd.d[j]),
            ...
            for j in 1..=m {
                for l in 1..=m {
                    ...
                    if !pz(&bd.a[j]) && !pz(&bd.w[l]) {
                        if j > l {
                            // only count the NEW cases
                            aw_all[0] += 1;
                            if !interlaces(&bd.a[j], &bd.w[l]) {
                                aw_all[1] += 1;
                                if aw_all[1] <= 3 {
                                    println!(
                                        "FAIL AW_all: A_{}≪W_{} board {:?}: {} vs {}",
                                        j,
                                        l,
                                        board,
                                        pf(&bd.a[j]),
                    ...
                    if !pz(&bd.d[j]) && !pz(&bd.w[l]) {
                        dw_all[0] += 1;
                        if !interlaces(&bd.d[j], &bd.w[l]) {
                            dw_all[1] += 1;
                            if dw_all[1] <= 3 {
                                println!(
                                    "FAIL DW: D_{}≪W_{} board {:?}: {} vs {}",
                                    j,
                                    l,
                                    board,
                                    pf(&bd.d[j]),
            ...
            for j in 1..=m {
                for l in j + 1..=m {
                    if !pz(&bd.d[j]) && !pz(&bd.d[l]) {
                        dd_fwd[0] += 1;
                        if !interlaces(&bd.d[j], &bd.d[l]) {
                            dd_fwd[1] += 1;
                            if dd_fwd[1] <= 3 {
                                println!(
                                    "FAIL DD_fwd: D_{}≪D_{} board {:?}: {} vs {}",
                                    j,
                                    l,
                                    board,
                                    pf(&bd.d[j]),
                        ...
                        dd_rev[0] += 1;
                        if !interlaces(&bd.d[l], &bd.d[j]) {
                            dd_rev[1] += 1;
                            if dd_rev[1] <= 3 {
                                println!(
                                    "FAIL DD_rev: D_{}≪D_{} board {:?}: {} vs {}",
                                    l,
                                    j,
                                    board,
                                    pf(&bd.d[l]),
                ...
                for j in 1..=mp {
                    for l in 1..=mp {
                        ...
                        if !pz(&a_plus) && !pz(&w_plus) {
                            aw_plus[0] += 1;
                            if !interlaces(&a_plus, &w_plus) {
                                aw_plus[1] += 1;
                                if aw_plus[1] <= 3 {
                                    println!(
                                        "FAIL AW+: A_{}^+≪W_{}^+ board {:?} m'={}",
                                        j, l, board, mp
                                    );
                                }
                            }
