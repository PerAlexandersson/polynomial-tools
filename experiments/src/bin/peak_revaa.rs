...
fn main() {
    ...
    for n in 2..=8 {
        ...
        for board in &boards {
            ...
            for j in 1..=m { a[j] = poly_add(&dp[j], &up[j]); }
            for j in 2..=m { for jp in 1..j {
                let aj = trim(&a[j]); let ajp = trim(&a[jp]);
                if aj.is_empty() || ajp.is_empty() { continue; }
                nt += 1;
                if interlaces_weak(&aj, &ajp) { np += 1; }
                else { if nt - np <= 3 { println!("  FAIL: board={:?}, j={}, j'={}", board, j, jp); } }
            }}
        }
        total += nt; pass += np;
