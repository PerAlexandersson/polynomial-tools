...
fn test_shape(mu: &[usize], d: usize, max_n: usize) -> (Option<usize>, usize) {
    ...
    for n in 0..=max_n {
        ...
        match check_rr(p) {
            ...
            None => { return (None, last_checked); } // overflow, can't check further
        }
...
fn main() {
    ...
    println!("{:<25} {:>5} {:>10} {:<20}", "mu", "mu_ell", "claw-free", "result");
    println!("{}", "-".repeat(65));
    ...
    for a in 1..=8 {
        for b in 1..=a {
            for c in 1..=b {
                ...
                let key = if cf { "3-col (claw-free)".to_string() }
                          else { format!("3-col mu_ell={}", c) };
        ...
        for b in 1..=a.min(5) {
            for c in 1..=b.min(4) {
                for e in 1..=c.min(3) {
                    ...
                    let key = if cf { "4-col (claw-free)".to_string() }
                              else { format!("4-col mu_ell={}", e) };
                    results.entry(key).or_default().push((mu, status));
                }
            }
        }
            ...
            for c in 1..=b.min(3) {
                for e in 1..=c.min(3) {
                    for f in 1..=e.min(2) {
                        ...
                        let key = if cf { "5-col (claw-free)".to_string() }
                                  else { format!("5-col mu_ell={}", f) };
                        results.entry(key).or_default().push((mu, status));
                    }
                }
            }
    ...
    for (section, cases) in &results {
        if section.contains("claw-free") { continue; }
        for (_, status) in cases {
            if status.starts_with("RR (n<=80)") { rr_count += 1; }
            else if status.starts_with("FAILS") { fail_count += 1; }
            else { overflow_count += 1; }
        }
    }
