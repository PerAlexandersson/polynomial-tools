        ...
        for board in &boards {
            ...
            for mu in &all_mu {
                ...
                for nu in &all_mu {
                    ...
                    // and diff >= 2
                    let diff: usize = (0..n).map(|i| nu[i] - mu[i]).sum();
                    if diff < 2 {
                        continue;
                    }
                    if (0..n).any(|i| nu[i] < mu[i]) {
                        continue;
                    }
                    if (0..n).any(|i| nu[i] > board[i]) {
                        continue;
                    }
                    ...
                    if h_nu.is_empty() || h_mu.is_empty() {
                        continue;
                    }

                    // H_mu - H_nu = (t-1) * sum_cofactors
                    // So sum_cofactors = (H_mu - H_nu) / (t-1)
                    let mut diff_poly = vec![0i64; h_mu.len().max(h_nu.len())];
                    for i in 0..h_mu.len() {
                        diff_poly[i] += h_mu[i];
                    }
                    for i in 0..h_nu.len() {
                    ...
                    if interlaces_weak(&sum_cof, &h_nu) {
                        ...
                        if total - pass <= 5 {
                            println!("  FAIL: board={:?} mu={:?} nu={:?}", board, mu, nu);
                            println!("    sum_cof={:?} H_nu={:?}", sum_cof, h_nu);
                        }
                    }
                }
            }
