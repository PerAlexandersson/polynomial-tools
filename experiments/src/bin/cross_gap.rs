...
fn main() {
    ...
    for n in 1..=max_n {
        for board in &gen_boards(n) {
            ...
            for col in 1..=m {
                for k in 1..=m { while s_col[col].len() <= k+1 { s_col[col].push(vec![0i64]); } s_col[col][k+1] = pa(&s_col[col][k], &a_jl[k][col]); }
                while t_col[col].len() <= mp+1 { t_col[col].push(vec![0i64]); }
                for k in (1..=m).rev() { t_col[col][k] = pa(&t_col[col][k+1], &w_jl[k][col]); }
            }
            // Test cross-group pairs for each l' with 2 ≤ l' ≤ m
            for lp in 2..=m { // l' is the last entry at lambda+ level
                let col1 = lp;   // upper group column
                let col2 = lp-1; // lower group column
                if col1 > m || col2 < 1 { continue; }
                // Upper group: k' > l', so k' = l'+1, ..., m+1
                // Lower group: j' < l', so j' = 1, ..., l'-1
                for kp in (lp+1)..=mp {
                    for jp in 1..lp {
                        // D_{k'}^+(col1) = S_{k'}^(col1), U_{j'}^+(col2) = T_{j'}^(col2)
                        let d_plus = if kp < s_col[col1].len() { &s_col[col1][kp] } else { &s_col[col1][m+1] };
                        let u_plus = if jp < t_col[col2].len() { &t_col[col2][jp] } else { &vec![0i64] };
                        // Ground truth
                        if !pz(d_plus) && !pz(u_plus) {
                            ground[0] += 1;
                            if !interlaces(d_plus, u_plus) { ground[1] += 1; }
                        }
                        // h = Σ_{i≥j'} D_{i,col2}
                        let mut h = vec![0i64];
                        for i in jp..=m { h = pa(&h, &d_jl[i][col2]); }
                        if pz(&h) { continue; }
                        // f = Σ_{i<k'} A_{i,col1} + Σ_{i≥j'} A_{i,col2}
                        let cross_part = if kp < s_col[col1].len() { s_col[col1][kp].clone() } else { s_col[col1][m+1].clone() };
                        let mut same_part = vec![0i64];
                        for i in jp..=m { same_part = pa(&same_part, &a_jl[i][col2]); }
                        let f = pa(&cross_part, &same_part);
                        // h ≪ f (full condition)
                        if !pz(&f) { h_f[0] += 1; if !interlaces(&h, &f) { h_f[1] += 1; } }
                        // h ≪ cross_part (= Σ A_{i,col1})
                        if !pz(&cross_part) { h_cross_a[0] += 1; if !interlaces(&h, &cross_part) { h_cross_a[1] += 1; } }
                        // h ≪ same_part (= Σ A_{i,col2})
                        if !pz(&same_part) { h_same_a[0] += 1; if !interlaces(&h, &same_part) { h_same_a[1] += 1; } }
                        // h ≪ U_{1,col1} = A_{1,col1}
                        if !pz(&a_jl[1][col1]) { h_u1_col1[0] += 1; if !interlaces(&h, &a_jl[1][col1]) { h_u1_col1[1] += 1; } }
                    }
                }
                // Also test D_{j',col2} ≪ U_{1,col1} for all j'
                for jp in 1..=m {
                    if !pz(&d_jl[jp][col2]) && !pz(&u_jl[1][col1]) {
                        dl_u1[0] += 1; if !interlaces(&d_jl[jp][col2], &u_jl[1][col1]) { dl_u1[1] += 1; }
                    }
                }
            }
        }
