...
fn find_roots(p: &[i64]) -> Vec<f64> {
    let d = p.len() - 1;
    if d == 0 { return vec![]; }
    if d == 1 { return vec![-(p[0] as f64) / (p[1] as f64)]; }
    // Companion matrix + QR
    let lc = p[d] as f64;
    let mut comp = vec![vec![0.0f64; d]; d];
    for i in 0..d-1 { comp[i+1][i] = 1.0; }
    for i in 0..d { comp[i][d-1] = -(p[i] as f64) / lc; }
...
fn qr_eigs(h: &mut Vec<Vec<f64>>) -> Vec<f64> {
    ...
    let mut sz = n;
    for _ in 0..10000 {
        if sz <= 1 { if sz == 1 { eigs.push(h[0][0]); } break; }
        if h[sz-1][sz-2].abs() < 1e-12 * (h[sz-1][sz-1].abs() + h[sz-2][sz-2].abs() + 1e-30) {
            eigs.push(h[sz-1][sz-1]); sz -= 1; continue;
        }
        let a=h[sz-2][sz-2]; let b=h[sz-2][sz-1];
        let c=h[sz-1][sz-2]; let d=h[sz-1][sz-1];
        let disc = ((a-d)*(a-d)+4.0*b*c).max(0.0).sqrt();
        let (e1,e2) = ((a+d+disc)/2.0, (a+d-disc)/2.0);
        let mu = if (e1-d).abs() < (e2-d).abs() { e1 } else { e2 };
        for i in 0..sz { h[i][i] -= mu; }
        let mut cs = Vec::new();
        for i in 0..sz-1 {
            let (a,b) = (h[i][i], h[i+1][i]);
            let r = (a*a+b*b).sqrt();
            if r < 1e-30 { cs.push((1.0,0.0)); continue; }
            let (c,s) = (a/r, b/r); cs.push((c,s));
            for j in 0..sz { let (t1,t2)=(h[i][j],h[i+1][j]); h[i][j]=c*t1+s*t2; h[i+1][j]=-s*t1+c*t2; }
        }
        for (i,&(c,s)) in cs.iter().enumerate() {
            for j in 0..sz { let (t1,t2)=(h[j][i],h[j][i+1]); h[j][i]=c*t1+s*t2; h[j][i+1]=-s*t1+c*t2; }
        }
        for i in 0..sz { h[i][i] += mu; }
    }
    while eigs.len() < n { eigs.push(h[eigs.len()][eigs.len()]); }
    eigs
}
    ...
    for n in 2..=5 {
        ...
        for board in &boards {
            ...
            fn gen_mu(board: &[usize], idx: usize,
                ...
                if idx == board.len() { result.push(mu.clone()); return; }
            ...
            for mu in &all_mu {
                for j in 0..n {
                    ...
                    let cj = trim(&cj);
                    if cj.len() <= 1 { continue; } // degree 0, trivial
                    ...
                    if this_sign_ok { sign_ok += 1; }
                    if this_dom_ok { dominance_ok += 1; }
                }
            }
        }
        println!("n={}: sign_alt {}/{}, dominance {}/{}",
