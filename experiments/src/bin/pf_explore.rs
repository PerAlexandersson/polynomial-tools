...
fn find_roots_f64(coeffs: &[f64]) -> Option<Vec<f64>> {
    ...
    if n == 1 { return Some(vec![-coeffs[0] / coeffs[1]]); }
    ...
    for _ in 0..n {
        ...
        q[deg - 1] = p[deg];
        for j in (0..deg - 1).rev() { q[j] = p[j + 1] + r * q[j + 1]; }
        p = q;
    }
    Some(roots)
}
...
fn newton_root(coeffs: &[f64]) -> Option<f64> {
    for &start in &[-1000.0, -100.0, -10.0, -5.0, -2.0, -1.0, -0.5, -0.1, 0.0, 0.5] {
        let mut x = start;
        for _ in 0..500 {
            let (val, deriv) = eval_pd(coeffs, x);
            if val.abs() < 1e-12 { return Some(x); }
            if deriv.abs() < 1e-15 { break; }
            x -= val / deriv;
        }
        let (val, _) = eval_pd(coeffs, x);
        if val.abs() < 1e-8 { return Some(x); }
    }
    None
}
...
fn eval_pd(coeffs: &[f64], x: f64) -> (f64, f64) {
    let mut val = 0.0;
    let mut deriv = 0.0;
    for i in (0..coeffs.len()).rev() { deriv = deriv * x + val; val = val * x + coeffs[i]; }
    (val, deriv)
}
...
fn check_interlaces(fc: &[i64], gc: &[i64]) -> bool {
    ...
    let (Some(mut fr), Some(mut gr)) = (find_roots_f64(&ff), find_roots_f64(&gf)) else { return false; };
...
fn main() {
    ...
    for i in 0..polys.len() - 1 {
        println!("P_{} ≼ P_{}: {}", i + 1, i + 2, check_interlaces(&polys[i], &polys[i + 1]));
    }
    println!("\n=== Reversed: P_{{n+1}} ≼ P_n ===");
    for i in 0..polys.len() - 1 {
        println!("P_{} ≼ P_{}: {}", i + 2, i + 1, check_interlaces(&polys[i + 1], &polys[i]));
    }
    println!("\n=== Recurrence search ===");
    use polynomial_tools::recurrence::{find_recurrence_adaptive, AdaptiveSearchOptions};
    let search = AdaptiveSearchOptions { max_rec_len: 3, max_var_deg: 3, max_idx_deg: 3, max_diff_deg: 2, verbose: false, ..Default::default() };
