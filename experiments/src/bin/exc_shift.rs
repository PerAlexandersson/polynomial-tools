...
fn shifted_exc_by_first(lambda: &[u8], s: i32) -> Vec<Vec<i64>> {
    ...
    for j in 1..=m {
        if (j as u8) > lambda[0] { continue; }
        let factor: Vec<i64> = if (j as i64) <= boundary { vec![1] } else { vec![0, 1] }; // 1 or t
        ...
        let mut sub = Vec::new();
        for i in 1..n { // original rows 2..n (0-indexed: 1..n-1)
...
fn main() {
    ...
    for n in 2..=max_n {
        ...
        for s in -1..=(n as i32) {
            ...
            let mut all_rr = true;
            for j in 1..=n {
                if !pz(&fj[j]) && !is_rr(&fj[j]) { all_rr = false; }
            }
            if !all_rr {
                print!("    F_j not all RR:");
                for j in 1..=n { if !pz(&fj[j]) { print!(" F_{}={}", j, format_poly(&pt(&fj[j]))); } }
                println!();
            }
        }
        println!();
    ...
    println!("=== Recursion check: does F^(s)(lambda, first=j) use F^(s) or F^(s+1) on sublambda? ===");
    ...
    for s in -1..=2 {
        ...
        for j in 1..=4 { total_from_fj = pa(&total_from_fj, &fj[j]); }
        let mat_full = shifted_exc_matrix(&board, s);
        let total_full = permanent(&mat_full);
        println!("  s={}: Σ F_j = {}  F^(s)(lam) = {}  match={}", s, format_poly(&pt(&total_from_fj)), format_poly(&pt(&total_full)), pt(&total_from_fj) == pt(&total_full));
    }
}
