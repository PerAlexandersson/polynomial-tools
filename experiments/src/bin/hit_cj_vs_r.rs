...
fn pscale(p: &[i64], c: i64) -> Vec<i64> { pt(&p.iter().map(|&x| x * c).collect::<Vec<_>>()) }
...
fn cofactor_poly(ideal: &[Vec<u8>], mu: &[u8], row_j: usize, col: usize) -> Vec<i64> {
    let n = mu.len(); let mut p = vec![0i64];
    for sigma in ideal { if sigma[row_j] as usize != col { continue; }
        let mut hits = 0; for i in 0..n { if i == row_j { continue; }
        ...
        for board in &gen_boards(n) {
            ...
            for mu in &sub_partitions(board) {
                for j in 0..n {
                    ...
                    let mut mu_prime = mu.clone(); mu_prime[j] += 1;
                    ...
                    if pz(&r) { continue; }
                    // R real-rooted?
                    r_rr[0] += 1; if is_real_rooted(&r) { r_rr[1] += 1; }
                    // C_j ≪ R?
                    cj_r[0] += 1; if interlaces(&c_j, &r) { cj_r[1] += 1; }
                    // R + s*C_j real-rooted for various s?
                    for (s, ctr) in [(2i64, &mut rs2), (5, &mut rs5), (100, &mut rs100)] {
                        let f = pa(&r, &pscale(&c_j, s));
                        ctr[0] += 1; if is_real_rooted(&f) { ctr[1] += 1; }
                    }
                }
            }
        }
