...
fn main() {
    ...
    let tests = [
        "D≪W(all)", "A≪W(j≤l)", "A≪W(all)", "U≪W(j≤l)", "U≪W(all)",
        "A≪tD(all)", "Σ_D≪U(all)", "R≪P",
        // At λ+ level:
        "S≪T(all)", "S≪S(rev)",
    ];
    for t in &tests { counts.insert(t, (0, 0)); }
    ...
    for n in 2..=7 {
        for board in &boards_312(n) {
            ...
            for j in 1..=m { for jp in 1..=m {
                let dj = trim(&dp[j]); let wjp = trim(&w[jp]);
                if dj.is_empty() || wjp.is_empty() { continue; }
                let e = counts.get_mut("D≪W(all)").unwrap();
                e.0 += 1; if interlaces_weak(&dj, &wjp) { e.1 += 1; }
            }}

            // A≪W(j≤l)
            for j in 1..=m { for l in j..=m {
                let aj = trim(&a[j]); let wl = trim(&w[l]);
                if aj.is_empty() || wl.is_empty() { continue; }
                let e = counts.get_mut("A≪W(j≤l)").unwrap();
                e.0 += 1; if interlaces_weak(&aj, &wl) { e.1 += 1; }
            }}
            ...
            // U≪W(j≤l)
            for j in 1..=m { for l in j..=m {
                let uj = trim(&up[j]); let wl = trim(&w[l]);
                if uj.is_empty() || wl.is_empty() { continue; }
                let e = counts.get_mut("U≪W(j≤l)").unwrap();
                e.0 += 1; if interlaces_weak(&uj, &wl) { e.1 += 1; }
            }}
            ...
            for j in 1..=m { for jp in 1..=m {
                let aj = trim(&a[j]); let tdj = trim(&poly_tmul(&dp[jp]));
                if aj.is_empty() || tdj.is_empty() { continue; }
                let e = counts.get_mut("A≪tD(all)").unwrap();
                e.0 += 1; if interlaces_weak(&aj, &tdj) { e.1 += 1; }
            }}
            ...
            for k in 1..=m_prime { for l in 1..=m_prime {
                let sk = trim(&s[k]); let tl = trim(&t_sum[l]);
                if sk.is_empty() || tl.is_empty() { continue; }
                let e = counts.get_mut("S≪T(all)").unwrap();
                e.0 += 1; if interlaces_weak(&sk, &tl) { e.1 += 1; }
            }}

            // S≪S(rev): S_k ≪ S_{k'} for k > k'
            for k in 2..=m_prime { for kp in 1..k {
                let sk = trim(&s[k]); let skp = trim(&s[kp]);
                if sk.is_empty() || skp.is_empty() { continue; }
                let e = counts.get_mut("S≪S(rev)").unwrap();
                e.0 += 1; if interlaces_weak(&sk, &skp) { e.1 += 1; }
            }}
        }
        print!("n={}: ", n);
    ...
    for (name, (total, pass)) in &items {
        let fail = total - pass;
        let status = if fail == 0 { "ALL PASS".to_string() } else { format!("{} FAIL", fail) };
