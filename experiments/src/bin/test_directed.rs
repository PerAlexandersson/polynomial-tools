...
fn main() {
    ...
    println!(
        "  f ≪ g (should be true):  {:?}",
        check_weak_interlacing(&f2, &g2)
    );
    println!(
        "  g ≪ f (should be false): {:?}",
        check_weak_interlacing(&g2, &f2)
    );
    ...
    println!(
        "  f ≪ g (should be None, deg(f) > deg(g)): {:?}",
        check_weak_interlacing(&f3, &g3)
    );
    println!(
        "  g ≪ f (should be true):  {:?}",
        check_weak_interlacing(&g3, &f3)
    );
    ...
    println!(
        "  A_3 ≪ t·A_3 (should be true): {:?}",
        check_weak_interlacing(&a3, &ta3)
    );
    println!(
        "  t·A_3 ≪ A_3 (should be None): {:?}",
        check_weak_interlacing(&ta3, &a3)
    );
}
