    ...
    for n in 2..=7 {
        ...
        for board in &boards {
            ...
            for k in 1..=m_prime { for l in 1..=m_prime {
                ...
                let tl = trim(&t_sum[l]);
                let sl = trim(&s[l]);
                if apk.is_empty() || wpl.is_empty() { continue; }
                
                // Full AW at λ+
                aw_plus.0 += 1;
                if interlaces_weak(&apk, &wpl) { aw_plus.1 += 1; }
                
                // A_k^+ ≪ T_l
                if !tl.is_empty() {
                    ak_tl.0 += 1;
                    if interlaces_weak(&apk, &tl) { ak_tl.1 += 1; }
                }
                
                // S_l ≪ A_k^+
                if !sl.is_empty() {
                    sl_ak.0 += 1;
                    if interlaces_weak(&sl, &apk) { sl_ak.1 += 1; }
                }
            }}
        }
        println!("n={}: AW+={}/{}, A+≪T={}/{}, S≪A+={}/{}",
