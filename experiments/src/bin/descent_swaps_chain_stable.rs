...
use polynomial_tools::real_rootedness::{is_real_rooted, check_weak_interlacing};
...
fn cabs(a: (f64,f64)) -> f64 { (a.0*a.0 + a.1*a.1).sqrt() }
...
fn main() {
    ...
    for n in 4..=max_n {
        ...
        for (&mask, class) in &by_descent {
            ...
            if is_chain { chain_ok += 1; }
            if is_stable { psi_stable += 1; }
            if is_chain && is_stable { both += 1; }
            ...
            if is_chain != is_stable {
                ...
                println!("  MISMATCH n={} S={}: chain={} stable={}", n, s_str, is_chain, is_stable);
            }
        }

        println!("n={}: chain={}/{} stable={}/{} both={}/{}",
            n, chain_ok, total, psi_stable, total, both, total);
    }

    println!("\nchain ↔ Psi stable: {}", if chain_ok == psi_stable && chain_ok == both { "EQUIVALENT" } else { "NOT equivalent" });
    println!("Total: chain={}/{} Psi_stable={}/{}", chain_ok, total, psi_stable, total);
}
