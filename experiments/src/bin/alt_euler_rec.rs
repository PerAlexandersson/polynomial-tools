...
fn main() {
    ...
    for (name, opts) in &configs {
        let unknowns =
            (opts.rec_len + 1) * (opts.var_deg + 1) * (opts.idx_deg + 1) * (opts.diff_deg + 1);
        ...
        match find_polynomial_recurrence(&polys, opts) {
            Some(rec) => {
                ...
                let max_coeff: usize = format!("{}", rec).len();
                if max_coeff > 200 {
    ...
    match find_recurrence_adaptive(&polys, &opts) {
        Some(res) => {
            ...
            println!(
                "Params: rec_len={} var_deg={} idx_deg={} diff_deg={}",
