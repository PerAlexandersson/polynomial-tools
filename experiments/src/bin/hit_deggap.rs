...
fn cofactor_poly(ideal: &[Vec<u8>], mu_prime: &[u8], row_j: usize, col: usize) -> Vec<i64> {
    ...
    for sigma in ideal {
        ...
        for i in 0..n {
            ...
            if sigma[i] as usize > mu_prime[i] as usize {
                hits += 1;
            }
        }
        while p.len() <= hits {
...
fn main() {
    ...
    for n in 2..=max_n {
        ...
        eprintln!(
            "n={}: {} boards done, {} deg-gap tests so far",
            n, board_count, deg_gap_tests
        );
    }

    println!("=== Degree gap lemma (n <= {}) ===", max_n);
    println!(
        "  Degree gap |deg C_j - deg C_{{j,k}}| <= 1: {}/{} ({} FAIL)",
        deg_gap_pass, deg_gap_tests, deg_gap_fail
    );
    println!("  Max gap seen: {}", max_gap_seen);
    println!(
        "  Cofactor interlacing: {}/{} ({} FAIL)",
        interl_pass,
        interl_tests,
        interl_tests - interl_pass
    );
}
