...
fn main() {
    // P_2: single edge 0-1
    let g = Graph::path(2);
    println!("G = P_2: {} vertices, edges: {:?}", g.num_vertices(), g.edges());

    // Delete edge (0,1): empty graph on 2 vertices -> chi = t^2 = [0, 0, 1]
    let g_del = g.delete_edge(0, 1);
    println!("G-e: {} vertices, edges: {:?}", g_del.num_vertices(), g_del.edges());
    ...
    println!("G/e: {} vertices, edges: {:?}", g_con.num_vertices(), g_con.edges());
