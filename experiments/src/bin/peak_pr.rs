...
fn roots_str(p: &[i64]) -> String {
    match find_real_roots(p) {
        ...
        Some(r) => format!(
            "{:?}",
            r.iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>()
        ),
        None => "NOT-RR".into(),
    }
}
...
fn main() {
    let max_n: usize = std::env::args()
        ...
        .unwrap_or(7);
    println!(
        "Testing P/R recursion on all Ferrers boards with n <= {}\n",
        max_n
    );
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for (i, &v) in r_poly.iter().enumerate() {
                tr[i + 1] = v;
            }
        ...
        println!(
            "n={}: boards={}, R_RR_fails={}, R≼P_fails={}, P≼R_fails={}, P+_RR_fails={}",
