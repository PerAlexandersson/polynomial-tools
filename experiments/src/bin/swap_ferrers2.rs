...
use polynomial_tools::real_rootedness::{check_interlacing_sturm, check_weak_interlacing, format_poly, is_real_rooted};
fn perms_on_board(lambda: &[u8]) -> Vec<Vec<u8>> {
    let n = lambda.len() as u8;
    if n == 0 { return vec![vec![]]; }
    all_permutations(n).into_iter().filter(|s| s.iter().enumerate().all(|(i,&v)| v <= lambda[i])).collect()
}
...
fn check_il(f: &[i64], g: &[i64]) -> &'static str {
    ...
    match check_interlacing_sturm(small, large) {
        Some(true) => "✓",
        Some(false) => match check_weak_interlacing(small, large) { Some(true) => "✓w", _ => "✗" },
        None => "?",
    }
}
fn main() {
    ...
    fn gen(n: u8, pos: usize, min_val: u8, current: &mut Vec<u8>, result: &mut Vec<Vec<u8>>) {
        if pos == n as usize { result.push(current.clone()); return; }
    ...
    for b in &boards {
        ...
        if !rr { all_rr = false; }
        let name: String = b.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(",");
        println!("{:<12} |S|={:<5} {:<50} {}", name, perms.len(), format_poly(&poly), if rr {"✓"} else {"✗"});
        ...
        for j in (i+1)..polys.len() {
            ...
            if la.iter().zip(lb.iter()).all(|(&a,&b)| a <= b) {
                if pa.len() > 1 && pb.len() > 1 {
                    ...
                    if r == "✗" {
                        fail_count += 1;
                        let na: String = la.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(",");
                        let nb: String = lb.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(",");
                        println!("  {} ≪ {}: ✗", na, nb);
                    }
                }
            }
    ...
    println!("Checked {} containment pairs, {} failures", check_count, fail_count);
}
