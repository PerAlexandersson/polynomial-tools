...
fn main() {
    let max_n: usize = std::env::args()
        ...
        .unwrap_or(7);
    println!(
        "=== Decomposition tests for AW at λ+ (j>l), n ≤ {} ===\n",
        max_n
    );
            ...
            for j in 1..=m {
                for l in 1..j {
                    ...
                    if !pz(&t_a[j]) && !pz(&w_l_plus) {
                        tj_wl[0] += 1;
                        if !exact_interlaces(&t_a[j], &w_l_plus) {
                            tj_wl[1] += 1;
                        }
                    }
                    // S_j ≪ W_l^+
                    if !pz(&s[j]) && !pz(&w_l_plus) {
                        sj_wl[0] += 1;
                        if !exact_interlaces(&s[j], &w_l_plus) {
                            sj_wl[1] += 1;
                        }
                    }
                }
