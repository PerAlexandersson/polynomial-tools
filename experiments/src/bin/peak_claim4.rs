...
fn main() {
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for k in 1..=m {
                ...
                if !pz(&t_arr[k]) {
                    tests[2] += 1;
                    if !exact_interlaces(&u[k], &t_arr[k]) {
                        fails[2] += 1;
                    }
                }
                ...
                if k < m && !pz(&t_arr[k + 1]) {
                    tests[9] += 1;
                    if !exact_interlaces(&t_arr[k + 1], &tuk) {
                        fails[9] += 1;
                    }
                }
            }
    ...
    for n in 2..=max_n {
        ...
        for board in &boards {
            ...
            for j in 1..=m {
                for l in j + 1..=m {
                    if pz(&u_p[j]) || pz(&u_p[l]) {
                        continue;
                    }
                    uu_fwd[0] += 1;
                    if !exact_interlaces(&u_p[j], &u_p[l]) {
                        uu_fwd[1] += 1;
                    }
                }
            }
    ...
    println!(
        "D_j≼U_l (ALL j,l): {}/{} pass {}",
        du_all[0] - du_all[1],
        du_all[0],
        if du_all[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "U_j≼U_l (j≤l, fwd): {}/{} pass {}",
        uu_fwd[0] - uu_fwd[1],
        uu_fwd[0],
        if uu_fwd[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "D_l≼D_j (j<l, rev): {}/{} pass {}",
        dd_rev[0] - dd_rev[1],
        dd_rev[0],
        if dd_rev[1] == 0 { "✓" } else { "FAIL" }
    );
    println!(
        "S_j≼T_l (ALL j,l):  {}/{} pass {}",
        st_all[0] - st_all[1],
        st_all[0],
        if st_all[1] == 0 { "✓" } else { "FAIL" }
    );
    ...
    for k in 1..=m {
        ...
        println!(
            "  (S_k+D_k)≼U_k: {}",
            if pz(&skdk) || pz(&u_arr[k]) {
                "trivial"
            } else if exact_interlaces(&skdk, &u_arr[k]) {
                "YES"
            } else {
                "NO"
            }
        ...
        println!(
            ...
            if pz(&q_k) || pz(&tu_k) {
                "trivial"
            } else if exact_interlaces(&q_k, &tu_k) {
                "YES"
            } else {
                "NO"
            }
