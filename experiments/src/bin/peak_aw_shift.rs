...
fn eval0(p: &[i64]) -> i64 { if p.is_empty() { 0 } else { p[0] } }
    ...
    for n in 2..=7 {
        ...
        for board in &boards {
            ...
            for k in 1..=m_prime {
                ...
                for j in 1..=m {
                    if j < k { sk = poly_add(&sk, &a_poly[j]); }
                    if j >= k { tk = poly_add(&tk, &w_poly[j]); }
                }
                s[k] = sk.clone(); t_sum[k] = tk.clone();
                a_plus[k] = poly_add(&sk, &tk);
            }
            
            for k in 1..=m_prime { for l in 1..=m_prime {
                let apk = trim(&a_plus[k]);
                if apk.is_empty() { continue; }
                
                // Compute h_{k,l}
                let h = if k > l {
                    // h = S_l + Σ_{j=l}^{k-1} D_j
                    let mut hh = s[l].clone();
                    for j in l..k { if j <= m { hh = poly_add(&hh, &dp[j]); } }
                    hh
                } else if k < l {
                    // h = S_k + Σ_{j=k}^{l-1} U_j
                    let mut hh = s[k].clone();
                    for j in k..l { if j <= m { hh = poly_add(&hh, &up[j]); } }
                    hh
                } else {
                    // h = S_l
                    s[l].clone()
                ...
                ntb += 1;
                if eval0(&apk) >= eval0(&h) { npb += 1; }
            }}
        }
        total += nt; pass += np; total_bd += ntb; pass_bd += npb;
