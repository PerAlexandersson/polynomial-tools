...
fn main() {
        ...
        for board in &gen_boards(n) {
            ...
            for mu in &subs {
                ...
                for k in 1..m {
                    ...
                    if sum != 0 { continue; } // doesn't divide
                    // Divide by (t-1)
    ...
    println!("With negative coefficients: {} ({:.1}%)", neg_count, 100.0 * neg_count as f64 / total as f64);
    println!("All non-negative: {} ({:.1}%)", total - neg_count, 100.0 * (total - neg_count) as f64 / total as f64);
}
