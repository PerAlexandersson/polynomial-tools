//! Search for "wild" exact-pattern subset sums on the right-base side that
//! best match the strong left-augmented auxiliary polynomials.
...
fn family_pattern_poly(
    ...
    p_b: u8,
    r: u8,
    target_pattern: EndpointPairPattern,
    n: u8,
) -> Vec<i64> {
    let Some(mask) = family_descent_set_mask else {
        return vec![0];
    ...
    for pi in permutations {
        ...
        let q = pi.iter().position(|&v| v == n - 1).unwrap() as u8 + 1;
        let _bin = classify_endpoint_relative_bin(q, p_a, p_b, r);
        values.push(
            compute(pi, Stat::Swaps)
                + usize::from(insertion_breaks_consecutive_ascending_pair_at_boundary(
                    pi,
...
fn subset_name(mask: usize) -> String {
    let mut pieces = Vec::new();
    for (i, pattern) in EndpointPairPattern::all().into_iter().enumerate() {
        if mask & (1 << i) != 0 {
...
fn main() {
    ...
    let max_n: u8 = args.first().and_then(|s| s.parse().ok()).unwrap_or(8);
    let run_chain_search = args.iter().any(|arg| arg == "--chains");

    let mut p2_counts: BTreeMap<usize, (usize, usize)> = BTreeMap::new();
    let mut p3_counts: BTreeMap<usize, (usize, usize)> = BTreeMap::new();
    let mut p4_counts: BTreeMap<usize, (usize, usize)> = BTreeMap::new();
    let mut s1_counts: BTreeMap<usize, (usize, usize)> = BTreeMap::new();
    let mut both_p2_p3_counts: BTreeMap<usize, (usize, usize)> = BTreeMap::new();
    let mut chain_pair_counts: BTreeMap<(usize, usize), (usize, usize)> = BTreeMap::new();
    ...
    for n in 4..=max_n {
        ...
        for target_descent_set_mask in 0u64..(1 << (n - 1)) {
            ...
            for consecutive_pair in valid_positions.windows(2) {
                ...
                let left_insertion_position = p_a;
                let right_insertion_position = p_b;
                ...
                let left_suffixes = {
                    ...
                    let mut running = vec![0];
                    for poly in left_bin_polys.iter().rev() {
                        running = poly_add(&running, poly);
                ...
                let mut pattern_polys = Vec::new();
                for pattern in EndpointPairPattern::all() {
                    pattern_polys.push(family_pattern_poly(
                        &permutations_by_descent_set,
                        right_base_mask,
                ...
                let mut subset_sums = vec![vec![0]; 1 << 8];
                for mask in 1usize..(1 << 8) {
                    let lsb = mask & (!mask + 1);
                    let bit_index = lsb.trailing_zeros() as usize;
                    let prev = mask ^ lsb;
                    subset_sums[mask] = poly_add(&subset_sums[prev], &pattern_polys[bit_index]);
                }

                for mask in 1usize..(1 << 8) {
                    let subset_poly = &subset_sums[mask];
                    if la_p2 != [0] && *subset_poly != [0] {
                        let entry = p2_counts.entry(mask).or_insert((0, 0));
                        entry.0 += 1;
                        if interlaces_weak(&la_p2, subset_poly) {
                            entry.1 += 1;
                        }
                    }
                    if la_p3 != [0] && *subset_poly != [0] {
                        let entry = p3_counts.entry(mask).or_insert((0, 0));
                        entry.0 += 1;
                        if interlaces_weak(&la_p3, subset_poly) {
                            entry.1 += 1;
                        }
                    }
                    if la_p4 != [0] && *subset_poly != [0] {
                        let entry = p4_counts.entry(mask).or_insert((0, 0));
                        entry.0 += 1;
                        if interlaces_weak(&la_p4, subset_poly) {
                            entry.1 += 1;
                        }
                    }
                    if la_s1 != [0] && *subset_poly != [0] {
                        let entry = s1_counts.entry(mask).or_insert((0, 0));
                        entry.0 += 1;
                        if interlaces_weak(&la_s1, subset_poly) {
                            entry.1 += 1;
                        }
                    }
                    if la_p2 != [0] && la_p3 != [0] && *subset_poly != [0] {
                        let entry = both_p2_p3_counts.entry(mask).or_insert((0, 0));
                        entry.0 += 1;
                        if interlaces_weak(&la_p2, subset_poly)
                            && interlaces_weak(&la_p3, subset_poly)
                        {
                            entry.1 += 1;
                        }
                    }
                ...
                if run_chain_search {
                    for mask_a in 1usize..(1 << 8) {
                        let poly_a = &subset_sums[mask_a];
                        if la_p2 == [0] || *poly_a == [0] {
                            continue;
                        }
                        for mask_b in 1usize..(1 << 8) {
                            if mask_a & !mask_b != 0 {
                                continue;
                            }
                            let poly_b = &subset_sums[mask_b];
                            if la_p3 == [0] || *poly_b == [0] {
                                continue;
                            }
                            let entry = chain_pair_counts.entry((mask_a, mask_b)).or_insert((0, 0));
                            entry.0 += 1;
                            if interlaces_weak(&la_p2, poly_a) && interlaces_weak(&la_p3, poly_b) {
                                entry.1 += 1;
                            }
                        }
                    }
    ...
    println!("=== Exact-pattern subset search ===");
    println!("total consecutive local cases: {}", total_cases);
    println!();

    let show_top = |title: &str, counts: BTreeMap<usize, (usize, usize)>| {
        ...
        println!("{}", title);
        for (mask, (checked, passed)) in vec.into_iter().take(16) {
            println!("  {:<48} {}/{}", subset_name(mask), passed, checked);
        }
        println!();
    };

    show_top("Best subsets for LA:P2:", p2_counts);
    show_top("Best subsets for LA:P3:", p3_counts);
    show_top("Best subsets for LA:P4:", p4_counts);
    show_top("Best subsets for LA:S1:", s1_counts);
    show_top(
        "Best subsets passing both LA:P2 and LA:P3:",
        both_p2_p3_counts,
    );

    if run_chain_search {
        let mut chain_vec: Vec<_> = chain_pair_counts
            .into_iter()
            .filter(|(_, (checked, _))| *checked >= 20)
            .collect();
        chain_vec.sort_by(|a, b| {
            let ar = a.1 .1 as f64 / a.1 .0 as f64;
            let br = b.1 .1 as f64 / b.1 .0 as f64;
            br.partial_cmp(&ar)
                .unwrap()
        ...
        println!("Best nested subset chains for LA:P2 then LA:P3:");
        for ((mask_a, mask_b), (checked, passed)) in chain_vec.into_iter().take(20) {
            println!(
                "  [{:<32}] subset [{:<32}] {}/{}",
                subset_name(mask_a),
                subset_name(mask_b),
