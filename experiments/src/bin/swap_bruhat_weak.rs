...
fn main() {
    for n in 3..=6u8 {
        ...
        for sigma in &all {
            ...
            let wrr = if wp.len() <= 2 { true } else { is_real_rooted(&wp) };
            let brr = if bp.len() <= 2 { true } else { is_real_rooted(&bp) };

            if wrr { weak_rr += 1; } else { weak_fail += 1; }
            if brr { bruhat_rr += 1; } else { bruhat_fail += 1; }
            ...
            if !wrr || !brr {
                println!(
                    ...
                    perm_str(sigma),
                    weak.len(), bruhat.len(),
                    format_poly(&wp), if wrr { "✓" } else { "✗" },
                    bruhat.len(),
                    format_poly(&bp), if brr { "✓" } else { "✗" },
                );
            }
        }

        println!("\nWeak:   {} real-rooted, {} failures out of {}", weak_rr, weak_fail, all.len());
        println!("Bruhat: {} real-rooted, {} failures out of {}\n", bruhat_rr, bruhat_fail, all.len());
    }
}
