...
fn poly_gcd(p: &Poly) -> i128 {
    ...
    let mut g = 0i128;
    for &c in p { g = gcd(g, c); }
    if g == 0 { 1 } else { g }
}

fn check_rr(p: &Poly) -> Option<bool> {
    if p.len() <= 2 { return Some(true); }
...
fn test_shape(mu: &[usize], d: usize, max_n: usize) -> (Option<usize>, usize) {
    ...
    for n in 0..=max_n {
        ...
        match check_rr(p) {
            ...
            Some(false) => { return (Some(n), n); }
            None => { return (None, last_checked); }
        }
...
fn main() {
    ...
    println!("Scanning Ferrers tiles with d={}, checking RR up to n={}", d, max_n);
    println!("{:<25} {:>3} {:>3} {:>5} {:<25}", "mu", "m1", "m2", "CF", "result");
    println!("{}", "-".repeat(70));
    ...
    for a in 1..=8 {
        for b in 1..=a {
            for c in 1..=b {
                ...
                if !cf {
                    let marker = if status.starts_with("RR") && !status.contains("FAILS") { " <<<" } else { "" };
                    println!("{:<25} {:>3} {:>3} {:>5} {:<25}{}",
                             format!("{:?}", mu), b, c, if cf {"yes"} else {"no"}, status, marker);
                }
            }
        }
    }
    ...
    println!("{:<12} {:>8} {:>8} {:>8}", "condition", "RR", "FAILS", "overflow");
    ...
    for ncols in 3..=4 {
        let ranges: Vec<Vec<usize>> = if ncols == 3 {
            let mut v = vec![];
            for a in 1..=8 { for b in 1..=a { for c in 1..=b { v.push(vec![a,b,c]); }}}
            v
        } else {
            let mut v = vec![];
            for a in 2..=6 { for b in 1..=a.min(5) { for c in 1..=b.min(4) { for e in 1..=c.min(3) { v.push(vec![a,b,c,e]); }}}}
            v
        };
        for mu in &ranges {
            ...
            let entry = counts.entry((cond.to_string(), cat.to_string())).or_insert(0u32);
            *entry += 1;
        }
    }
    for cond in &["m1>m2", "m1=m2"] {
        let rr = counts.get(&(cond.to_string(), "RR".to_string())).unwrap_or(&0);
        let fail = counts.get(&(cond.to_string(), "FAILS".to_string())).unwrap_or(&0);
        let ovfl = counts.get(&(cond.to_string(), "overflow".to_string())).unwrap_or(&0);
