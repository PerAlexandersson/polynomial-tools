...
fn main() {
    ...
    println!("=== DU gap (exact, GCD-fixed, paper convention), n ≤ {} ===\n", max_n);
    ...
    println!("(5t^2+6t+1) ≪ (10t^2+2t): {}", exact_interlaces(&[1,6,5], &[0,2,10]));
    println!();

    let mut total = 0u64; let mut valid = 0u64;
    let mut aw_ord = [0u64;2]; let mut ww_ord = [0u64;2];
    let mut du_all = [0u64;2]; let mut uu_ord = [0u64;2];
    let mut aw_ext = [0u64;2]; let mut dd_fwd = [0u64;2]; let mut dd_rev = [0u64;2];
    let mut uu_rev = [0u64;2]; let mut st_all = [0u64;2]; let mut st_gap = [0u64;2];
    ...
    for n in 1..=max_n {
        ...
        for board in &boards {
            ...
            for j in 1..=m{for l in j..=m{
                if !pz(&a[j])&&!pz(&w[l]){aw_ord[0]+=1;if !exact_interlaces(&a[j],&w[l]){aw_ord[1]+=1;
                    if aw_ord[1]<=3{println!("FAIL AW({},{}) {:?}",j,l,board);}}}
                if j<l&&!pz(&w[j])&&!pz(&w[l]){ww_ord[0]+=1;if !exact_interlaces(&w[j],&w[l]){ww_ord[1]+=1;
                    if ww_ord[1]<=3{println!("FAIL WW({},{}) {:?}",j,l,board);}}}
                if !pz(&u[j])&&!pz(&u[l]){uu_ord[0]+=1;if !exact_interlaces(&u[j],&u[l]){uu_ord[1]+=1;
                    if uu_ord[1]<=3{println!("FAIL UU({},{}) {:?}: {} vs {}",j,l,board,pf(&u[j]),pf(&u[l]));}}}
            }}
            for j in 1..=m{for l in 1..=m{
                if !pz(&d[j])&&!pz(&u[l]){du_all[0]+=1;if !exact_interlaces(&d[j],&u[l]){du_all[1]+=1;
                    if du_all[1]<=3{println!("FAIL DU({},{}) {:?}",j,l,board);}}}
            }}
            for j in 1..=m{for l in 1..j{
                if !pz(&a[j])&&!pz(&w[l]){aw_ext[0]+=1;if !exact_interlaces(&a[j],&w[l]){aw_ext[1]+=1;
                    if aw_ext[1]<=5{println!("FAIL AW_ext({},{}) {:?}: {} vs {}",j,l,board,pf(&a[j]),pf(&w[l]));}}}
                if !pz(&u[j])&&!pz(&u[l]){uu_rev[0]+=1;if !exact_interlaces(&u[j],&u[l]){uu_rev[1]+=1;
                    if uu_rev[1]<=3{println!("FAIL UU_rev({},{}) {:?}: {} vs {}",j,l,board,pf(&u[j]),pf(&u[l]));}}}
            }}
            for j in 1..=m{for l in j+1..=m{
                if !pz(&d[j])&&!pz(&d[l]){
                    dd_fwd[0]+=1;if !exact_interlaces(&d[j],&d[l]){dd_fwd[1]+=1;}
                    dd_rev[0]+=1;if !exact_interlaces(&d[l],&d[j]){dd_rev[1]+=1;}
                }
            }}
            for j in 1..=m{for l in 1..=m{
                if !pz(&s[j])&&!pz(&t_a[l]){
                    st_all[0]+=1;if !exact_interlaces(&s[j],&t_a[l]){st_all[1]+=1;
                        if st_all[1]<=3{println!("FAIL S≪T({},{}) {:?}",j,l,board);}}
                    if j>l{st_gap[0]+=1;if !exact_interlaces(&s[j],&t_a[l]){st_gap[1]+=1;}}
                }
            }}
        }
    ...
    println!("Theorem:");
    show("(a) A_j≪W_l j≤l",aw_ord); show("(b) W_j≪W_l j≤l",ww_ord);
    show("(c) D_j≪U_l all",du_all); show("(d) U_j≪U_l j≤l",uu_ord);
    println!("\nStrengthenings:");
    show("A_j≪W_l j>l",aw_ext); show("D_j≪D_l fwd",dd_fwd);
    show("D_l≪D_j rev",dd_rev); show("U_j≪U_l j>l rev",uu_rev);
    println!("\nλ+ level:");
    show("S_j≪T_l all",st_all); show("S_j≪T_l j>l GAP",st_gap);
}
