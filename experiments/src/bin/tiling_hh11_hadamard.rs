...
fn main() {
    ...
    for n in 0..=max_n {
        for r in 0..=n/2 {
            ...
            for j in 0..=n as i64 {
                ...
                let c = binom(j + 1, s);
                if c > 0 && min_j.is_none() { min_j = Some(j); }
            ...
            if coeffs.len() <= 1 || coeffs.iter().all(|&c| c == 0) { continue; }
            g_total += 1;
    ...
    if g_failures == 0 {
        println!("  All g_{{n,r}} real-rooted for n <= {} ({} polynomials checked)", max_n, g_total);
    } else {
    ...
    for h in 2..=6 {
        ...
        for n in 0..=max_n {
            for r in 0..=n/2 {
                ...
                while coeffs.len() > 1 && *coeffs.last().unwrap() == 0 { coeffs.pop(); }
                if coeffs.len() <= 2 || coeffs.iter().all(|&c| c == 0) { continue; }
                q_total += 1;
                if !is_real_rooted(&coeffs) {
    ...
    for h in 2..=6 {
        ...
        if ok { println!("  h={}: All P_n real-rooted (n <= {})", h, max_n); }
    }
}
