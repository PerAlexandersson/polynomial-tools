...
fn main() {
    ...
    println!("mu={:?}, d={}", mu, d);
    println!("P_n(1) [total tilings]: {}", p1.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
    ...
    for n in 2..=max_n {
        let mut rhs = p1[n];
        for j in 1..n { rhs -= b1[j] * p1[n - j]; }
        b1[n] = rhs;
    }
    let b_ff: Vec<i128> = b1.iter().skip(2).copied().take_while(|&x| x != 0).collect();
    println!("B(x,1)-x coeffs [fault-free counts]: {}", b_ff.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
    ...
    for n in 1..=max_n {
        for j in 1..=n {
            if b_dense[j] != 0 { q1[n] += b_dense[j] * q1[n - j]; }
        }
    }
    println!("Q_n(1) [dense tilings]: {}", q1.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
    ...
    let s1_coeffs: Vec<i128> = s_polys.iter().map(|p| if p.len() > 1 { p[1] } else { 0 }).collect();
    println!("S_n'(1) [total row-1 tiles]: {}", s1_coeffs.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
    ...
    println!("S_n(0) [tilings avoiding row 1]: {}", s0_coeffs.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(","));
}
