...
fn to_i64_poly(p: &[BigInt]) -> Option<Vec<i64>> {
    ...
    p.iter().map(|c| (c / &g).to_i64()).collect()
}
...
fn bpoly_add(a: &[BigInt], b: &[BigInt]) -> Vec<BigInt> {
    ...
    while r.last().map_or(false, |c| c.is_zero()) {
        r.pop();
    }
    r
}
...
fn main() {
    ...
    for n in 2..=9 {
        ...
        if n == 2 {
            process_tree(
                ...
                &mut tab_rr,
                &mut a_interlaces_b,
                &mut a_weak_interlaces_b,
                &mut a_interlaces_ab,
                &mut a_weak_interlaces_ab,
                &mut b_weak_interlaces_tab,
                &mut a_weak_interlaces_tab,
                &mut ab_weak_interlaces_tab,
                &mut leaf_del_count,
                &mut leaf_del_interlace,
                &mut leaf_del_weak_interlace,
            );
            continue;
        }
    ...
    println!(
        "  A_v ≪ B_v (strict):         {}/{}",
        a_interlaces_b, total_vertices
    );
    println!(
        "  A_v ≪ B_v (weak):           {}/{}",
        a_weak_interlaces_b, total_vertices
    );
    println!(
        "  A_v ≪ A_v+B_v (strict):     {}/{}",
        a_interlaces_ab, total_vertices
    );
    println!(
        "  A_v ≪ A_v+B_v (weak):       {}/{}",
        a_weak_interlaces_ab, total_vertices
    );
    println!(
        "  B_v ≪ t*A_v+B_v (weak):     {}/{}",
        b_weak_interlaces_tab, total_vertices
    );
    println!(
        "  A_v ≪ t*A_v+B_v (weak):     {}/{}",
        a_weak_interlaces_tab, total_vertices
    );
    println!(
        "  A_v+B_v ≪ t*A_v+B_v (weak): {}/{}",
        ab_weak_interlaces_tab, total_vertices
    );
    println!();
    println!("Leaf deletion interlacing:");
    println!(
        "  S_{{L(T')}} ≪ S_{{L(T)}} (strict): {}/{}",
        leaf_del_interlace, leaf_del_count
    );
    println!(
        "  S_{{L(T')}} ≪ S_{{L(T)}} (weak):   {}/{}",
        leaf_del_weak_interlace, leaf_del_count
    );
}
...
fn process_tree(
    ...
    ab_rr: &mut u64,
    tab_rr: &mut u64,
    a_interlaces_b: &mut u64,
    a_weak_interlaces_b: &mut u64,
    a_interlaces_ab: &mut u64,
    a_weak_interlaces_ab: &mut u64,
    b_weak_interlaces_tab: &mut u64,
    a_weak_interlaces_tab: &mut u64,
    ...
    for &v in &order {
        ...
        *total_vertices += 1;
        ...
        if let Some(true) = interlaces_big(av, bv) {
            *a_interlaces_b += 1;
        }
        if let Some(true) = weak_interlaces_big(av, bv) {
            *a_weak_interlaces_b += 1;
        }
        if let Some(true) = interlaces_big(av, &abv) {
            *a_interlaces_ab += 1;
        }
        if let Some(true) = weak_interlaces_big(av, &abv) {
            *a_weak_interlaces_ab += 1;
        }
        if let Some(true) = weak_interlaces_big(bv, &tabv) {
            *b_weak_interlaces_tab += 1;
        }
        if let Some(true) = weak_interlaces_big(av, &tabv) {
            *a_weak_interlaces_tab += 1;
        }
        if let Some(true) = weak_interlaces_big(&abv, &tabv) {
            *ab_weak_interlaces_tab += 1;
        }
    }
