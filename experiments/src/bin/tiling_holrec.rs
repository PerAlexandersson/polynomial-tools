...
fn find_holonomic(polys: &[Poly], order: usize, max_n_deg: usize, max_t_deg: usize) -> bool {
    ...
    for n in order..polys.len() {
        ...
        for p in 1..=max_n_deg {
            n_powers[p] = n_powers[p - 1] * nn;
        }
    ...
    for &fc in &free_cols {
        ...
        for n in order..polys.len() {
            ...
            for p in 1..=max_n_deg {
                n_powers[p] = n_powers[p - 1] * (n as i128);
            }

            // For each t-power, check the sum is 0
            let max_r_check =
                polys.iter().take(n + 1).map(|p| deg(p)).max().unwrap_or(0) + max_t_deg;
        ...
        if verified {
            println!(
                "  FOUND! order={}, n-deg={}, t-deg={}",
                order, max_n_deg, max_t_deg
            );
            for j in 0..=order {
                let mut terms = vec![];
                for p in 0..=max_n_deg {
...
fn main() {
    ...
    println!(
        "Generated {} polynomials, max deg = {}",
        polys.len(),
        polys.iter().map(|p| deg(p)).max().unwrap_or(0)
    );
