...
fn main() {
    for h in 2..=4 {
        ...
        for p in 0..=3 {
            let label = if p == 0 { "M (original)".to_string() }
                        else { format!("M^({})", p) };
        ...
        for n in 0..=max_n {
            for j in 0..=max_j {
                ...
                let m2_n1 = if n > 0 { m_peeled(h as i64, (n-1) as i64, j as i64, 1) } else { 0 };
                if m_orig != m2_n + m2_n1 {
                    println!("  MISMATCH at n={}, j={}: {} != {} + {}", n, j, m_orig, m2_n, m2_n1);
                    ok = false;
                }
            }
        }
        if ok { println!("  M = (I+S)*M^(2) verified. ✓"); }
        ...
        for n in 0..=max_n {
            for j in 0..=max_j {
                ...
                let m3_n1 = if n > 0 { m_peeled(h as i64, (n-1) as i64, j as i64, 2) } else { 0 };
                if m2 != m3_n + m3_n1 {
        ...
        if ok2 { println!("  M^(2) = (I+S)*M^(3) verified. ✓"); }

        // Print M^(base) = M^(j+1) for a few entries
        println!("\n  Base matrix M^(j+1)_{{n,j}} = C(h(n-2j)/2, j):");
        println!("  {:>4} | {:>5} {:>5} {:>5} {:>5} {:>5}", "n", "j=0", "j=1", "j=2", "j=3", "j=4");
        for n in 0..=16i64 {
            let row: Vec<String> = (0..=4).map(|j: i64| {
                let m = n - 2*j;
                if m >= 0 && m % 2 == 0 {
