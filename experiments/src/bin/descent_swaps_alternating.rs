...
fn check_compat(f: &[i64], g: &[i64]) -> bool {
    ...
    let weights: Vec<(i64, i64)> = vec![
        (1,1),(1,2),(2,1),(1,3),(3,1),(1,5),(5,1),(2,3),(3,2),(1,7),(7,1),
    ];
...
fn main() {
    let max_n: u8 = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(14);
    ...
    for n in 1..=max_n {
        ...
        let h_polys: Vec<(u8, Vec<i64>)> = by_pos.iter().enumerate()
        ...
        println!("═══ n = {} ({} alt perms, {} positions) ═══", n, alt.len(), h_polys.len());
        ...
        if let Some(ref ph) = prev_h {
            let t_prev: Vec<i64> = {
                ...
                for (i, &c) in ph.iter().enumerate() { v[i + 1] = c; }
                v
            };
        ...
        if let Some(ref ph) = prev_h {
            if ph.len() > 1 && h_n.len() > 1 {
                let (small, large) = if ph.len() <= h_n.len() { (ph, &h_n) } else { (&h_n, ph) };
            ...
            for i in 0..h_polys.len() {
                for j in (i+1)..h_polys.len() {
                    if !check_compat(&h_polys[i].1, &h_polys[j].1) {
                        all_compat = false;
                        println!("  COMPAT FAIL: H_{},{} vs H_{},{}",
                            n, h_polys[i].0, n, h_polys[j].0);
                    }
                }
            }
            if all_compat { println!("  Compatible family ✓"); }
        }
        ...
        if h_polys.len() >= 2 {
            ...
            for i in 0..cum_polys.len() - 1 {
                ...
                if c1.len() > 1 && c2.len() > 1 {
                    let (s, l) = if c1.len() <= c2.len() { (c1, c2) } else { (c2, c1) };
            ...
            if chain_ok { println!("  Cumulative chain ✓"); }
        }
        ...
        if n >= 4 && h_polys.len() >= 2 {
            ...
            for (j, _) in &h_polys {
                ...
                let count_alt_source: usize = alt.iter()
                    .filter(|sigma| sigma.iter().position(|&v| v == n).unwrap() as u8 + 1 == p)
                    .filter(|sigma| {
                        // Remove n from position p, get π
                        let mut pi: Vec<u8> = sigma.iter()
                            .filter(|&&v| v != n)
                            .copied()
                            .collect();
                        // Check if π is alternating (up-down)
                        if pi.len() < 2 { return true; }
                        for i in 0..pi.len() - 1 {
                            if i % 2 == 0 && pi[i] >= pi[i + 1] { return false; }
                            if i % 2 == 1 && pi[i] <= pi[i + 1] { return false; }
                        }
                        true
                    })
                    .count();

                let total_at_j: usize = alt.iter()
                    .filter(|sigma| sigma.iter().position(|&v| v == n).unwrap() as u8 + 1 == p)
                    .count();
                ...
                if total_at_j > 0 {
                    ...
                    if p == n {
                        ...
                        print!("  p={}: {}/{} from A_{{n-1}} ({:.0}%)",
                            p, count_alt_source, total_at_j, frac * 100.0);
                        if frac == 1.0 { println!(" ✓"); } else { println!(" ✗"); }
                    } else if frac > 0.0 {
                        println!("  p={}: {}/{} from A_{{n-1}} ({:.0}%)",
                            p, count_alt_source, total_at_j, frac * 100.0);
                    }
                }
            }
