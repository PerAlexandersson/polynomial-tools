...
fn main() {
        ...
        for board in &gen_boards(n) {
            ...
            for l in 1..=m {
                for j in 1..=m {
                    let td = pmt(&d_jl[j][l]);
                    // tD_j << U_j (same index)
                    if !pz(&td) && !pz(&u_jl[j][l]) {
                        td_u_same[0] += 1;
                        if !interlaces(&td, &u_jl[j][l]) {
                            td_u_same[1] += 1;
                        }
                    }
                    for jp in 1..=m {
                        // tD_j << U_{j'} (all)
                        if !pz(&td) && !pz(&u_jl[jp][l]) {
                            td_u_all[0] += 1;
                            if !interlaces(&td, &u_jl[jp][l]) {
                                td_u_all[1] += 1;
                            }
                        }
                        // tD_j << W_{j'} (all)
                        if !pz(&td) && !pz(&w_jl[jp][l]) {
                            td_w[0] += 1;
                            if !interlaces(&td, &w_jl[jp][l]) {
                                td_w[1] += 1;
                            }
                        }
                        // A_j << tD_{j'} for j <= j'
    ...
    println!(
        "tD_j << U_j (same j): {}/{} pass ({} FAIL)",
        td_u_same[0] - td_u_same[1],
        td_u_same[0],
        td_u_same[1]
    );
    println!(
        "tD_j << U_{{j'}} (all): {}/{} pass ({} FAIL)",
        td_u_all[0] - td_u_all[1],
        td_u_all[0],
        td_u_all[1]
    );
    println!(
        "tD_j << W_{{j'}} (all): {}/{} pass ({} FAIL)",
        td_w[0] - td_w[1],
        td_w[0],
        td_w[1]
    );
    println!(
        "A_j << tD_{{j'}} (j<=j'): {}/{} pass ({} FAIL)",
        a_td[0] - a_td[1],
        a_td[0],
        a_td[1]
    );
