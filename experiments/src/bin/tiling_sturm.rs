...
fn check_interleaves(f: &[i64], g: &[i64]) -> Option<bool> {
    ...
    if df == 0 { return Some(true); } // 0 or const << anything
    if dg == 0 { return Some(true); } // anything << 0 or const
    ...
    if df + 1 == dg {
        ...
        for i in 0..df {
            ...
            if rf[i] > rg[i + 1] { return Some(false); }
        }
        Some(true)
    } else {
...
fn main() {
    ...
    println!("mu={:?}, d={}, checking Sturm sequence up to n={}", mu, d, max_n);
    ...
    for n in 0..=max_n {
        ...
        let rr = match &p64 {
            Some(v) if d_n >= 2 => {
                ...
                if !r && first_fail_rr.is_none() { first_fail_rr = Some(n); }
                if r { "yes" } else { "NO" }
            }
            Some(_) => "triv",
            None => "ovfl",
        ...
        let il = if let (Some(prev), Some(curr)) = (&prev_p64, &p64) {
            match check_interleaves(prev, curr) {
                Some(true) => "yes",
                Some(false) => {
                    if first_fail_il.is_none() && n > 1 { first_fail_il = Some(n); all_interleave = false; }
                    "NO"
                }
                None => "N/A",
            }
    ...
    if let Some(n) = first_fail_il {
        ...
        println!("  All consecutive pairs interlace (Sturm sequence) up to n={}", max_n);
    }
}
