...
fn main() {
    ...
    for n in 4..=max_n {
        ...
        for pi in &perms_prev {
            ...
            prev_list.push(pi);
            prev_by_des.entry(descent_set_bitmask(pi)).or_default().push(idx);
        }
        ...
        for (&mask, _) in &by_descent {
            ...
            if !one_not_in_s { continue; } // Only 1 not in S
            ...
            for &p in &vp {
                ...
                if let Some(sp) = sp_d {
                    if let Some(indices) = prev_by_des.get(&sp) {
                        for &idx in indices {
                            let pi = prev_list[idx];
                            let q = pi.iter().position(|&v| v == n - 1).unwrap() as u8 + 1;
                            desc_by_q.entry(q).or_default().push(compute(pi, Stat::Swaps));
                        }
                    }
                }

                let all_qs: Vec<u8> = {
                    let mut qs: Vec<u8> = asc_by_q.keys().chain(desc_by_q.keys()).copied().collect();
                    qs.sort(); qs.dedup(); qs
                };
                ...
                if p == 2 {
                    ...
                    for &q in &all_qs {
                        ...
                        let a_vals: Vec<usize> = asc_by_q.get(&q).map(|v|
                            v.iter().filter(|(_, e)| !e).map(|(s, _)| *s).collect()
                        ).unwrap_or_default();
                        if !a_vals.is_empty() {
                        ...
                        let c_vals: Vec<usize> = asc_by_q.get(&q).map(|v|
                            v.iter().filter(|(_, e)| *e).map(|(s, _)| *s).collect()
                        ).unwrap_or_default();
                        if !c_vals.is_empty() {
                        ...
                        // D_q: descent source
                        let d_vals: Vec<usize> = desc_by_q.get(&q).map(|v| v.clone()).unwrap_or_default();
                        if !d_vals.is_empty() {
                ...
                for i in 0..polys.len() {
                    for j in (i+1)..polys.len() {
                        if !check_pairwise_compatible(polys[i], polys[j]) {
                            if fail_count < 3 {
                                println!("    {} vs {}: {} vs {}",
                                    all_source_polys[i].0, all_source_polys[j].0,
                                    format_poly(polys[i]), format_poly(polys[j]));
                            }
                            fail_count += 1;
                        }
                    }
                }
                if fail_count > 3 { println!("    ... and {} more failures", fail_count - 3); }
            }

            // Also check split source at p=2 alone
            if has_p2 {
                let p2_polys: Vec<&Vec<i64>> = all_source_polys.iter()
        ...
        println!("n={}: split@p2 {}/{}, joint {}/{}",
            n, n_split_ok, n_split_tested, n_joint_ok, n_joint_tested);
    }

    println!("\n=== SUMMARY ===");
    println!("Split source at p=2: {}/{}", total_split_ok, total_split_tested);
    println!("Joint interlacing (all sources): {}/{}", total_joint_ok, total_joint_tested);
}
