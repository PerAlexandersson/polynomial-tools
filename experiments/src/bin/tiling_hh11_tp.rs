...
use polynomial_tools::{check_total_positivity, is_real_rooted, check_interlacing};
...
fn main() {
    ...
    for h in 2..=max_h {
        ...
        print!("  TP (minors up to {}x{}, n <= {}): ", max_minor, max_minor, max_n);
        ...
        if rr_ok { println!("  Real-rootedness: PASS (n <= {})", max_n); }
        if interl_ok { println!("  Interlacing: PASS (n <= {})", max_n); }
        println!();
    }
}
