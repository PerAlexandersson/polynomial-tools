...
fn pf(p: &[i64]) -> String {
    ...
    for (i,&c) in p.iter().enumerate() { if c==0{continue;} match(c,i){
        ...
        (-1,1)=>t.push("-t".into()),(c,1)=>t.push(format!("{}t",c)),
        (1,e)=>t.push(format!("t^{}",e)),(-1,e)=>t.push(format!("-t^{}",e)),
    ...
    for n in 1..=max_n {
        ...
        for board in &boards {
            ...
            for j in 1..=m { for l in j..=m {
                if !pz(&a[j]) && !pz(&w[l]) {
                    aw_ord[0]+=1; if !interlaces(&a[j],&w[l]) { aw_ord[1]+=1;
                        if aw_ord[1]<=3 { println!("FAIL AW({},{}) {:?}: {} vs {}", j,l,board,pf(&a[j]),pf(&w[l])); }
                    }
                }
                if j<l && !pz(&w[j]) && !pz(&w[l]) {
                    ww_ord[0]+=1; if !interlaces(&w[j],&w[l]) { ww_ord[1]+=1;
                        if ww_ord[1]<=3 { println!("FAIL WW({},{}) {:?}", j,l,board); }
                    }
                }
                if !pz(&u[j]) && !pz(&u[l]) {
                    uu_ord[0]+=1; if !interlaces(&u[j],&u[l]) { uu_ord[1]+=1;
            ...
            for j in 1..=m { for l in 1..=m {
                if !pz(&d[j]) && !pz(&u[l]) {
                    du_all[0]+=1; if !interlaces(&d[j],&u[l]) { du_all[1]+=1;
                        if du_all[1]<=3 { println!("FAIL DU({},{}) {:?}: {} vs {}", j,l,board,pf(&d[j]),pf(&u[l])); }
                    }
                }
            }}
            ...
            for j in 1..=m { for l in 1..j {
                if !pz(&a[j]) && !pz(&w[l]) {
                    aw_ext[0]+=1; if !interlaces(&a[j],&w[l]) { aw_ext[1]+=1;
            ...
            for j in 1..=m { for l in 1..=m {
                if !pz(&d[j]) && !pz(&w[l]) {
                    dw_all[0]+=1; if !interlaces(&d[j],&w[l]) { dw_all[1]+=1;
                        if dw_all[1]<=3 { println!("FAIL DW({},{}) {:?}", j,l,board); }
                    }
                }
            }}
            for j in 1..=m { for l in j+1..=m {
                if !pz(&d[j]) && !pz(&d[l]) {
                    dd_fwd[0]+=1; if !interlaces(&d[j],&d[l]) { dd_fwd[1]+=1; }
                    dd_rev[0]+=1; if !interlaces(&d[l],&d[j]) { dd_rev[1]+=1; }
                }
                if !pz(&u[j]) && !pz(&u[l]) {
            ...
            for j in 1..=m { for l in 1..j {
                if !pz(&u[j]) && !pz(&u[l]) {
                    uu_rev[0]+=1; if !interlaces(&u[j],&u[l]) { uu_rev[1]+=1;
            ...
            for j in 1..=m { for l in 1..=m {
                if !pz(&s[j]) && !pz(&t_arr[l]) {
                    st_all[0]+=1;
                    if !interlaces(&s[j], &t_arr[l]) { st_all[1]+=1;
                        if st_all[1]<=3 { println!("FAIL S≪T({},{}) {:?}: {} vs {}", j,l,board,pf(&s[j]),pf(&t_arr[l])); }
                    }
                    if j > l {
                        st_gap[0]+=1;
                        if !interlaces(&s[j], &t_arr[l]) { st_gap[1]+=1; }
                    }
                }
            }}
            ...
            for mp in 1..=m {
                for j in 1..=mp { for l in 1..=mp {
                    ...
                    if !pz(&ap) && !pz(&wp) {
                        aw_plus_all[0]+=1;
                        if !interlaces(&ap, &wp) { aw_plus_all[1]+=1;
                            if aw_plus_all[1]<=3 { println!("FAIL AW+({},{}) {:?} m'={}", j,l,board,mp); }
                        }
                    }
                }}
            }
        }
        println!("n={}: {}/{} boards 312-avoiding", n, valid_boards, total_boards);
    }
