...
use polynomial_tools::{is_real_rooted, check_interlacing, check_weak_interlacing, format_poly};
use std::env;
...
fn decode_state(mut idx: usize, ell: usize, d: usize) -> Vec<usize> {
    ...
    for i in (0..ell).rev() { state[i] = idx % base; idx /= base; }
    state
}
...
fn analyze(mu: &[usize], d: usize, max_n: usize) {
    ...
    for j in 0..ns {
        ...
        for v in 0..=d {
            ...
            if ok {
                ...
                new_state[0] = v;
                for k in 1..ell { new_state[k] = old[k - 1]; }
    ...
    for n in 0..=max_n {
        ...
        let rr = match &p64 {
            Some(v) if deg >= 2 => {
                if is_real_rooted(v) { "RR" } else { "NOT-RR" }
            }
            Some(_) => "triv",
            None => "overflow",
        };
        ...
        if deg <= 6 {
            ...
            println!("P_{:>3}: deg={:>3}, P(1)={:<20}  {}  {}", n, deg, total, rr, interlace);
        }
        ...
        for i in 0..ns {
            for j in 0..ns {
                if poly_is_zero(&t_mat[i][j]) || poly_is_zero(&vec_state[j]) { continue; }
