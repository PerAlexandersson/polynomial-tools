...
fn find_recurrence(polys: &[Poly], order: usize, max_deg_a: usize) -> Option<Vec<Poly>> {
    ...
    for j in 0..=order {
        ...
        while aj.len() > 1 && *aj.last().unwrap() == 0 {
            aj.pop();
        }
        coeffs.push(aj);
    }
    ...
    for order in 1..=13 {
        for max_deg in 0..=4 {
            if let Some(coeffs) = find_recurrence(&polys, order, max_deg) {
                println!(
                    "Found recurrence of order {} with max t-degree {}:",
                    order, max_deg
                );
...
fn print_interlacing_depth(polys: &[Poly]) {
    ...
    for k in 1..=12 {
        ...
        match first_fail {
            None => println!(
                "  k={:>2}: P_n << P_{{n+{}}} holds for all {} compatible pairs up to n={}",
                k, k, n_checked, last_ok
            ),
            Some((n, df, dg)) => println!("  k={:>2}: FAILS at n={} (deg {}→{})", k, n, df, dg),
        }
    }
}
