...
fn main() {
    let max_n: usize = std::env::args()
        ...
        .unwrap_or(7);
    println!(
        "Testing (mP-R) ≼ tR on all Ferrers boards with n <= {}\n",
        max_n
    );
        ...
        for board in &boards {
            ...
            if !poly_is_zero(&q) && !poly_is_zero(&tr) {
                if !interlaces(&q, &tr) {
                    interl_fails += 1;
                    if interl_fails <= 5 {
                        let qr = find_real_roots(&q)
                            .map(|r| format!("{:.4?}", r))
                            .unwrap_or("NOT-RR".into());
                        let trr = find_real_roots(&tr)
                            .map(|r| format!("{:.4?}", r))
                            .unwrap_or("NOT-RR".into());
                        println!(
                            "FAIL: Q ≼ tR for {:?}: Q={} roots={}, tR={} roots={}",
                            board,
                            poly_fmt(&q),
                            qr,
                            poly_fmt(&tr),
    ...
    println!("\n=== SUMMARY ===");
    println!(
        "Total: {}, Q=mP-R not RR: {}, tR not RR: {}, Q≼tR fails: {}, P+ not RR: {}",
