...
fn run_polymake(script: &str, timeout_secs: u64) -> Option<(Vec<i64>, f64)> {
    ...
    let hstar: Vec<i64> = stdout
        .lines()
        .find(|l| l.starts_with("HSTAR:"))?
        .strip_prefix("HSTAR: ")?
        .split_whitespace()
    ...
    let cpu = stdout
        ...
        .and_then(|l| {
            let before = &l[..l.find("CPU")?];
            before[before.rfind('=')? + 1..].trim().parse::<f64>().ok()
        })
        .unwrap_or(0.0);
    Some((hstar, cpu))
}
...
fn main() {
    ...
    bench(
        "shape [3,2,1]",
        &Poset::from_shape(&Partition::new(vec![3, 2, 1])),
        true,
        true,
    );
    ...
    bench(
        "shape [4,3,2,1]",
        &Poset::from_shape(&Partition::new(vec![4, 3, 2, 1])),
        true,
        true,
    );
    bench(
        "shape [3,3,3]",
        &Poset::from_shape(&Partition::new(vec![3, 3, 3])),
        true,
        true,
    );
    ...
    bench(
        "shape [5,4,3,2,1]",
        &Poset::from_shape(&Partition::new(vec![5, 4, 3, 2, 1])),
        true,
        true,
    );
    bench(
        "shape [4,4,4,4]",
        &Poset::from_shape(&Partition::new(vec![4, 4, 4, 4])),
        true,
        true,
    );
    ...
    bench(
        "shape [5,5,5]",
        &Poset::from_shape(&Partition::new(vec![5, 5, 5])),
        true,
        true,
    );
    ...
    bench(
        "shape [5,5,5,5]",
        &Poset::from_shape(&Partition::new(vec![5, 5, 5, 5])),
        false,
        false,
    );
    bench(
        "shape [6,5,4,3,2,1]",
        &Poset::from_shape(&Partition::new(vec![6, 5, 4, 3, 2, 1])),
        false,
        false,
    );
    bench(
        "shape [6,6,6]",
        &Poset::from_shape(&Partition::new(vec![6, 6, 6])),
        false,
        false,
    );
    bench(
        "shape [6,6,6,6]",
        &Poset::from_shape(&Partition::new(vec![6, 6, 6, 6])),
        false,
        false,
    );
    bench(
        "shape [7,6,5,4,3,2,1]",
        &Poset::from_shape(&Partition::new(vec![7, 6, 5, 4, 3, 2, 1])),
        false,
        false,
    );
}
